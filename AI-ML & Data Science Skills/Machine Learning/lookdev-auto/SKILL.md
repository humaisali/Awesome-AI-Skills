---
name: lookdev-auto
description: "Automated visual tuning: a vision or video model rates rendered variants in a loop. Render several labeled variants into one artifact, ask the model to rate them and suggest better values, render the suggestions, ask it to pick the best, repeat until good â€” the model is the eye, you run the loop."
risk: safe
...
Use whenever "looks/feels right" is the success criterion and there's no cheap numeric metric â€” animation easing/timing, zoom/camera feel, color grade, layout/spacing, design params, render/encoder settings, prompt params. Use the automated counterpart to lookdev when there's no human to sit the loop.

_Source: [connerkward/lookdev-auto-skill](https://github.com/connerkward/lookdev-auto-skill) (MIT)._

# Visual eval loop â€” let a vision/video model tune what only an eye can judge
...
1. **Render N labeled variants into ONE artifact.** Vary the parameter(s) across a
   ...
   "A Â· 2.2Hz Â· Î¶0.5"). Images â†’ a labeled grid/contact sheet. Video/motion â†’ a
...
2. **One model call, structured output.** Send the single artifact with an explicit
   rubric (define what "good" means â€” and what "too much"/"too little" look like).
   Ask for **per-variant ratings + concrete suggested new values as JSON**:
   `{"ratings":{"A":n,...},"best_so_far":"X","suggest":[[p1,p2],...]}`.
3. **Coarse â†’ fine.** Round 1 = wide spread to locate the region. Round 2 = render the
   model's suggestions (+ carry the current best) into one artifact; ask it to **pick
   the single best**. Usually converges in **2 rounds**.
4. **Stop when sufficient** â€” best rates high and suggestions cluster. Apply the winner.

## Token / quality / step reductions (do these)

- **One artifact per round, not one call per variant.** The biggest saver â€” a 6-variant
  round is 1 upload + 1 inference, not 6. Montage/grid beats a loop of single calls.
- **Burn params onto the artifact.** The model sees label+result together â†’ no separate
  "variant A used X" context to carry â†’ fewer tokens, fewer mistakes.
...
- **Calibration anchors.** Include one deliberately-bad and one safe-default variant as
  fixed anchors each round â€” gives the model a reference scale and exposes when its
...
- **Independent rubric, stated up front.** Define "good" concretely in the prompt
  (smooth, subtle settle, not bouncy, not sluggish). Don't ask "which do you like" â€”
...
- **Reuse renders across rounds.** Carry the round-1 winner's clip into round 2 instead
  of re-rendering it.
- **Early-exit.** If round-1 top â‰¥9/10 and the three suggestions are within a small delta,
  skip round 2.
- **Cheapest judge that can see the failure.** Frames-through an image VLM can judge
  ...
  thing being judged is **temporal** (easing, timing, motion smoothness) â€” those are
  invisible in stills.

## When NOT to use it

- A real numeric metric exists and correlates with quality â†’ optimize that directly;
  don't pay a model per step.
- The judgment is subjective-to-the-user (their taste, brand) â†’ show them the variants
  and let them pick; a model's "best" isn't their best. (This is why the screen-studio
  spring auto-tune was dropped â€” the model's pick didn't match the owner's eye.)
- One or two variants â†’ just look yourself.

## Caveats (learned)

- The model's pick is an *opinion*, not ground truth â€” anchor it, and sanity-check the
  winner against the safe default yourself before committing.
- Vision/video models perceive gross differences well, fine ones poorly â€” keep variant
