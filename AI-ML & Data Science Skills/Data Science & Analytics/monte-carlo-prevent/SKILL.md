...
- References or opens a `.sql` file or dbt model (files in `models/`) â†’ run Workflow 1
- Mentions a table name, dataset, or dbt model name in passing â†’ run Workflow 1

- Describes a planned change to a model (new column, join update, filter change, refactor) â†’ **STOP â€” run Workflow 4 before writing any code**
-
- Adds a new column, metric, or output expression to an existing
  model â†’ run Workflow 4 first, then ALWAYS offer Workflow 2
  regardless of risk tier â€” do not skip the monitor offer
- Asks about data quality, freshness, row counts, or anomalies â†’ run Workflow 1
- Wants to triage or respond to a data quality alert â†’ run Workflow 3

Present the results as context the engineer needs before proceeding â€” not as a response to a question.
...
Jinja references â€” if absent, do not activate.

### Macros and snapshots â€” gate edits, skip auto-context
...
macros are inlined into every model that calls them at compile time â€” a one-line
...
This applies whenever the user expresses intent to modify a model â€” including phrases like:

- "I want to add a columnâ€¦"
- "Let me add / I'm addingâ€¦"
- "I'd like to change / update / renameâ€¦"
- "Can you add / modify / refactorâ€¦"
- "Let's addâ€¦" / "Add a `<column>` column"
- Any other description of a planned schema or logic change
- "Exclude / filter out / remove [records/customers/rows]â€¦"
- "Adjust / increase / decrease [threshold/parameter/value]â€¦"
- "Fix / bugfix / patch [issue/bug]â€¦"
- "Revert / restore / undo [change/previous behavior]â€¦"
- "Disable / enable [feature/logic/flag]â€¦"
- "Clean up / remove [references/columns/code]â€¦"
- "Implement [backend/feature] forâ€¦"
- "Create [models/dbt models] forâ€¦" (when modifying existing referenced tables)
- "Increase / decrease / change [max_tokens/threshold/date constant/numeric parameter]â€¦"
...
**Do not write or edit any SQL until the change impact assessment (Workflow 4) has been presented to the user.** The assessment must come first â€” not after the edit, not in parallel.

---

## Pre-edit gate â€” check before modifying any file
...
1. Has the synthesis step been run for THIS SPECIFIC CHANGE in the
   current prompt?
2. **If YES** â†’ proceed with the edit
3. **If NO** â†’ stop immediately, run Workflow 4, present the full
   ...
   **If risk is Low:** use judgment â€” proceed if straightforward
...
being changed in the current prompt â€” not just general table health.

Example:

- âœ… "Given 34 downstream models depend on is_paying_workspace,
  adding 'MC Internal' to the exclusion list will exclude these
  workspaces from all downstream health scores and exports.
  Confirm?"
- âŒ "Workflow 4 already ran. Making the edit now."
...
the change") â€” proceed but note the skipped assessment.
...
| `getMonitors`                | Monitor configs â€” filter by table using mcons array                  |
...
### 4. Change impact assessment â€” REQUIRED before modifying a model
...
**What:** Generates 3-5 targeted SQL queries to verify the change behaved as intended. Uses Workflow 4 context â€” requires both impact assessment and file edit in session.
...
Do NOT say: "Proceeding with the edit." â€” that skips the engineer's decision.
...
(Use the model filename without .sql extension â€” NOT "acme.analytics.orders" or "prod.public.client_hub")
...
the assessment): Be strict â€” only emit markers for tables whose lineage **and**
...
Be looser â€” emit markers for all tables the assessment meaningfully covered, even
...
marker when the gap is specifically about the columns or logic being changed â€”
