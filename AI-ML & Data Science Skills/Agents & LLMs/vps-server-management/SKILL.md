...
Source of truth: `library/infrastructure.md` (read it for the latest â€” IPs/expirations change).

## Servers (Hostinger VPS) â€” 3 total

| Hostname | IP | OS | Purpose | Expires |
|---|---|---|---|---|
| openclaw-server | <IP> | Ubuntu 24.04 (Dokploy) | OpenClaw â€” personal instance | <expiry> |
| n8n-server | <IP> | Ubuntu 24.04 (n8n) | All n8n workflow automations (primary) | <expiry> |
| hermes-server | <IP> | Ubuntu 24.04 | Hermes Agent â€” Discord gateway (Vilnius, LT) | <expiry> |

SSH as `root@<IP>`.

## Access levels (never share higher than needed)

1. **App login** â€” e.g. `app.example.hstgr.cloud`. Build/edit workflows, no server access. Safest to share.
2. **VPS SSH** â€” `root@<IP>`. Docker, files, system config. Trusted technical people only.
3. **Hostinger hPanel** â€” `hpanel.hostinger.com`. Billing, reboot, OS reinstall. Exposes SSH creds + browser terminal, so it grants server access too. The user only.

## Managing a VPS via an agent

For multi-step or exploratory work, **SSH into the box first and launch the agent ON the VPS** (e.g. `codex --yolo`), then talk to that local-on-server agent â€” it has full filesystem/process context and avoids fragile SSH round-trips. For short command sequences (update, config change, restart), driving an existing SSH session directly (e.g. via a cmux pane) is fine.
...
- **OpenClaw** â†’ openclaw-server (managed via Dokploy).
- **Hermes** â†’ hermes-server (Discord gateway). Setup/config docs in `library/hermes/`.
- **n8n** â†’ n8n-server.
...
- **Default model** lives in `~/.hermes/config.yaml` under `model.provider` + `model.default` â€” NOT in `.env`. Change via `hermes model` (interactive) or edit the yaml directly, then `hermes gateway restart` to propagate to gateways.
- npm `EBADENGINE` warnings during update (deps want Node >=24, box runs v22) are non-blocking â€” do not "fix" them.
