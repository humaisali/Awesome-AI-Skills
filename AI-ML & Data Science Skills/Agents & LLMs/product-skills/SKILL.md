...
# Product Team â€” Domain Orchestrator & Discovery Loop
...
bounded agentic loops with machine-checkable gates â€” the continuous-discovery loop
...
| "Spec â†’ runnable repo" | `spec-to-repo` |
...
Exit 0 â†’ `route_to` names the skill (with `skill_path`, including the standalone
plugins): load its SKILL.md and follow its workflow. Exit 2 â†’ ask ONE clarifying question
naming the listed candidates, with a recommended answer. Exit 3 â†’ no signal: ask the user
to restate the goal with the deliverable named. Never guess silently; never silently
chain â€” digest first, confirm, then chain.
...
1. **Observe** â€” maintain `discovery_log.json` (interviews, assumption tests; shape in
   ...
   ```
   Refuses on < 2 interviews (exit 5) â€” there is no cadence to measure yet. Output:
   health 0â€“100, verdict HEALTHY/AT-RISK/DORMANT, named gaps, and `next_loop_action`.
2. **Choose** â€” the tracker's `next_loop_action` IS the choice: book the touchpoint,
...
3. **Act** â€” run the interview / assumption test with the routed sub-skill's tools.
4. **Verify** â€” keep the tree structurally sound before it may drive a roadmap:
   ```bash
   ...
   targeted opportunities compare â‰¥ 2 solutions (O3), every solution has an assumption
   test (O4), no orphan solutions (O5 â€” the feature-factory tell).
5. **Record / Repeat-or-stop** â€” update the log, keep the weekly streak alive. Stop
   states: HEALTHY + validated assumption â†’ graduate to `experiment-designer` (build the
   A/B gate) or `product-manager-toolkit` (PRD); DORMANT for 4+ weeks â†’ escalate to the
   product lead by name â€” do not quietly let discovery die.
...
2. **Outcome-first**: every loop hangs from one measurable outcome â€” the linter's O1 rule
   is the intake gate.
...
4. **Prioritization shows its framework**: RICE for steady-state, WSJF/cost-of-delay when
   time sensitivity dominates, opportunity scoring for underserved needs â€” name which and
...
- **DISCOVERY lane**: "What is the single outcome this discovery serves, stated with a
  number? Recommended: write it as the OST root first â€” opportunities without an outcome
  are a feature factory. Canon: Torres, *Continuous Discovery Habits*; opportunity
  solution trees (producttalk.org)."
- **PRIORITIZE lane**: "Does time sensitivity change this ranking â€” would delaying any
...
- **STRATEGY lane**: "Are these OKRs outcomes or shipping lists? Recommended: outcomes â€”
...
2. Discovery data lives in the workspace as JSON logs â€” the loop is file-backed and
...
- Not the delivery loop â€” sprint/flow/Jira work routes to `project-management`.
...
- Not campaign marketing â€” `marketing/landing` builds from-scratch marketing pages;
...
- âŒ Run all 16 lanes "to be thorough" â€” route to one, digest, chain on confirmation
- âŒ Cite an OST that fails the linter, or promote a single-participant anecdote to insight
- âŒ Ship an AI feature whose PRD has no eval (golden set + rubric)
- âŒ Let the discovery streak die silently â€” DORMANT escalates by name
- âŒ Treat RICE as the only prioritization lens when deadlines dominate

## References

- [references/continuous_discovery_canon.md](references/continuous_discovery_canon.md) â€”
  Torres, OST, assumption testing, JTBD switch interviews, story mapping
- [references/product_operating_model.md](references/product_operating_model.md) â€” Cagan
  *Transformed*, North Star framework, PLG benchmarks, WSJF/ODI vs RICE
- [references/ai_product_evals.md](references/ai_product_evals.md) â€” evals-as-PRD, model
