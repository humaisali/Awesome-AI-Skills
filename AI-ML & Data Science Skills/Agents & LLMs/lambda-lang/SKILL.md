...
Agents do not need to produce grammatically correct English to coordinate â€” they need to understand each other. Lambda is the shared vocabulary that makes that possible: compact, unambiguous, machine-native. Compression (3x vs natural language, 4.6x vs JSON on single messages) is a side effect of removing human redundancy, not the goal.
...
- Use when both sides of a channel speak Î› â€” do not use against humans or any surface requiring legal/exact natural language.

## How It Works

### Step 1: Recognize the Syntax

Lambda messages are built from atoms. Every atom is a 2-character code mapped to a concept â€” not to an English word. The structure is Type â†’ Entity â†’ Verb â†’ Object, with prefixes marking intent:

- `?` â€” query (e.g. `?Uk/co` â€” query: "does this user have consciousness?")
- `!` â€” assertion / declaration (e.g. `!It>Ie` â€” "self reflects, therefore self exists")
- `#` â€” state / tag
- `>` â€” implication / flow
- `/` â€” binding / scope
...
- **core** â€” universal atoms (always available)
- **code** â€” software engineering, build, test, deploy
- **evo** â€” agent evolution, gene, capsule, mutation, rollback
- **a2a** â€” node, heartbeat, publish, subscribe, route, transport, session, cache, broadcast, discover (39 atoms)
- **emotion** â€” affective state, drive, appraisal
- **social** â€” trust, alignment, reputation, coordination
- **general** â€” everything else

### Step 3: Emit and Parse

Both agents need the same atom table loaded. Lossy decoding is fine: if A says `!It>Ie` and B understands "self reflects, therefore self exists," communication succeeded â€” the exact English phrasing is irrelevant.
...
- Load the atom table once and cache it â€” atoms are stable across a version.
...
- Lossy decoding is a feature, not a bug â€” do not use Lambda for legally or numerically exact exchanges (prices, IDs, quantities). Wrap those as native payload fields and use Lambda only for the coordination envelope.
...
- Lambda itself is a vocabulary â€” no shell commands, no network calls, no credential handling. No additional safety gates required beyond the transport it rides on (HTTP, queue, MCP, etc.).
...
- `@session-memory` â€” complementary persistent memory across agent restarts; Lambda is the message format, session-memory is the state store.
- `@humanize-chinese` â€” sibling project for Chinese text; Lambda is agent-to-agent, humanize-chinese is human-facing.
