...
- **Coding (default) â†’ Codex CLI.** Strongest coding agent, especially for complex, long-running SWE tasks. It's on an unlimited-usage plan â€” effectively unlimited, don't ration it.
- **Most other tasks â†’ Pi Agent** (`pi` in a cmux terminal). All Pi agents run opus-4.8-fast via OpenRouter at xhigh reasoning effort.
- **Frontend / design â†’ Pi.** Opus 4.8 Fast beats Codex on UI, styling, design.
...
1. **ONE single line â€” never newlines in the message body.** In a TUI, newline = Enter: a multi-line prompt submits at the first line and the rest arrives as fragmented mid-turn steering messages. Use ". " or "; " instead of line breaks, then one explicit enter. For long instructions, write them to a file and send: `read /tmp/task.md and follow it`.
2. **Wrap the prompt in plain double quotes â€” NEVER escaped.** `cmux send --surface surface:N "your prompt"`. The recurring bug is emitting `\"` â€” in bash that's literal-broken and dies with `unexpected EOF`. Inside the prompt, avoid apostrophes and literal double quotes (write "dont", "wont", "lets"); rephrase instead of escaping. If a send failed, the cause was the escaped `\"`, not the quote type.
...
Claude Code note: after it finishes, it may prefill a predicted next user message â€” that draft is Claude, not the user.
...
- **Hermes** (Nous Research, Python): persistent autonomous agent â€” cross-session memory, built-in scheduler, 40+ tools; can orchestrate the other CLIs as workers. Skills: `~/.hermes/skills/`.
