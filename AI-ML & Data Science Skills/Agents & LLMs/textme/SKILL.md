---
name: textme
description: "Text Claude from your phone â€” set up the njerschow/textme daemon so inbound iMessages drive a Claude Code session on your laptop, with voice notes, image input, code execution, and a phone-number whitelist."
category: automation
risk: critical
...
- Pair with [[sendblue-notify]] for bidirectional flow â€” outbound completion pings + inbound commands on the same Sendblue account.
...
- Node.js 18+.
- An active Sendblue account with API credentials and a provisioned iMessage number â€” set up via [[sendblue-cli]] (`sendblue setup`, then `sendblue show-keys` to surface API key/secret).
...
1. Send `status` from the whitelisted phone â€” should get a directory + state reply.
...
Wire outbound completion pings via [[sendblue-notify]] *and* inbound control via textme â€” they share the same Sendblue account but solve opposite problems:

- Claude finishes a long task â†’ texts user via [[sendblue-notify]] (`Stop` hook).
- User replies "look at the diff" â†’ textme routes that into Claude â†’ Claude responds back via Sendblue.
...
- âœ… **Whitelist exactly one phone number to start.** The whitelist is the security boundary; expand it slowly and only to numbers the user controls.
- âœ… **Run the daemon as a regular user**, never as root or via `sudo`.
- âœ… **Start in a sandbox directory** for first tests (`cd ~/textme-sandbox`), not in `~` or a real repo.
- âœ… **Verify the non-whitelist ignore path** before enabling auto-start. A daemon that processes any sender is an open shell on the host.
- âœ… **Keep `pollIntervalMs` â‰¥ 5000** unless the user understands the Sendblue rate limits and cost implications.
- âŒ **Don't share the Sendblue number publicly.** Even with a whitelist, the host is doing per-message work; a flood from an unknown sender still costs polling cycles.
- âŒ **Don't store `config.json` in a repo, dotfiles backup, or cloud sync** â€” it contains API credentials and the user's phone number.
- âŒ **Don't run the daemon on a shared machine** without considering what every other user of that machine can now reach by sending an SMS.
...
- **The daemon polls on an interval** â€” there is no push delivery. Expect single-digit-second latency between message receipt and Claude response.
...
- **Lock-screen previews of replies leak.** When Claude responds to a message, the reply lands on the user's lock screen unredacted. Don't ask Claude over textme to surface secrets, tokens, or customer data via SMS â€” link to a local log or PR instead.
...
- **Confusing textme with the Sendblue MCP.** They look similar but the MCP variant only gives Claude *outbound* Sendblue tools â€” it does not open an inbound channel. If the user wants "text Claude from my phone", they need textme; if they want "Claude can send a text mid-session", the MCP is lighter.

## Related Skills

- `@sendblue-notify` â€” Outbound counterpart (Claude â†’ phone). Composes with textme to make the loop bidirectional.
- `@sendblue-cli` â€” Account setup, credential management, and the `show-keys` command that surfaces the API key/secret textme needs.
- `@sendblue-api` â€” HTTP API reference for users who want to build a custom inbound handler instead of using textme's daemon.
- `@update-config` â€” If wiring textme alongside Claude Code hooks (e.g. a `Stop` hook that pings the user), use this for the settings.json edits.
