---
name: "pm-skills"
description: "Use when coordinating project-delivery work across the 8 project-management sub-skills â€” sprint/velocity analytics, portfolio health, Jira/JQL, Confluence, Atlassian admin, templates, meeting analysis, team comms. Triggers on 'our sprints feel off', 'project health report', 'audit our Jira permissions', 'when will it be done', 'run the delivery loop'. Forks context to route to one sub-skill via a deterministic signal router and returns a digest; can also drive a full goalâ†’planâ†’executeâ†’verifyâ†’close delivery loop through the repo-wide agent-harness with Jira MCP data bridged into the domain's analytics tools. Distinct from product-team (what to build vs how to deliver it), business-operations (internal ops), and engineering/agent-harness (the generic loop engine this orchestrator plugs into)."
context: fork
version: 2.11.1
...
# Project Management â€” Domain Orchestrator & Delivery Loop
...
**Looping:** turn a delivery goal into a bounded agentic loop â€” pull live Jira data via the
...
Run the router â€” do not eyeball the table when a script can decide:

```bash
python3 scripts/pm_goal_router.py --text "<the goal>" --output json
```

Exit 0 â†’ `route_to` names the sub-skill: load its SKILL.md and follow its workflow.
Exit 2 â†’ ask ONE clarifying question naming the listed candidates, with a recommended
answer. Exit 3 â†’ no signal: ask the user to restate the goal with the deliverable named.
Never guess silently; never silently chain a second sub-skill â€” digest first, confirm, then
chain.

## The delivery loop (agentic)

For goals (not questions) â€” "get sprint 14 to a verified close", "produce a portfolio
health report from live Jira", "make our flow metrics visible weekly" â€” run the
loop-library contract (Observe â†’ Choose â†’ Act â†’ Verify â†’ Record â†’ Repeat-or-stop):

1. **Observe** â€” pull fresh state: `mcp__atlassian__searchJiraIssuesUsingJql` (get
   ...
   (refuses on < 10 completed items â€” thin history forecasts are lies).
2. **Choose** â€” route the next task with `pm_goal_router.py`; one task at a time.
3. **Act** â€” execute with the routed sub-skill's own tools per its SKILL.md.
4. **Verify** â€” gate the plan and every close with:
   ```bash
   ...
   ```
   Plus each sub-skill's own gates (scrum-master's â‰¥ 3-sprints rule, atlassian-admin's
   VERIFY steps). Never adjudicate your own verification.
5. **Record / Repeat-or-stop** â€” for multi-task goals, run the state through the repo-wide
   ...
   stagnated. An exhausted budget is an escalation â€” never a success report.
...
2. **Acceptance must be machine-checkable** â€” a command, or a criterion with a threshold.
...
4. **Never modify a gate you are judged by** â€” same locked-evaluator invariant as
   autoresearch-agent.
5. **Forecasts are ranges with confidence, never dates** â€” Monte Carlo percentiles
   (p50/p70/p85/p95), per Vacanti. Single-date promises are the anti-pattern.
6. **Max 3 attempts per task, 12 loop iterations per goal** â€” then escalate to the named
...
- **SPRINT lane**: "Do you want to *measure* flow (cycle time, WIP, throughput, age) or
  *forecast* delivery? Recommended: measure first â€” a forecast off unmeasured flow is
...
- **HEALTH lane**: "Is your project status self-reported RAG or derived from signals?
  ...
  self-report â€” that diff finds watermelon projects. Canon: Kanban Guide 2025;
...
- **ADMIN lane**: "Is this action reversible, and who approves it? Recommended: name the
  approver before touching permissions â€” admin actions are approval-required terminal
...
2. Jira/Confluence access goes through the bundled MCP; capabilities NOT in
   ...
   creation, admin config) are done in the web UI â€” never invent tool names.
3. Inputs may be partial â€” every tool ships `--sample` so the shape is visible first.

## Non-goals

- Not a replacement for the sub-skills â€” the orchestrator routes and loops; the
  sub-skills do the work.
- Not the generic loop engine â€” that is `engineering/agent-harness`; this orchestrator is
  the PM-domain adapter (data bridge + governance gate + lane router).
- Does not decide *what* to build â€” that's `product-team`.

## Output artifacts

| Mode | Artifact |
|---|---|
| Route | Sub-skill's own artifact + â‰¤ 200-word digest with one canon-cited challenge |
...
- âŒ Run all 8 sub-skills "to be thorough" â€” route to one, digest, chain on confirmation
- âŒ Report sprint health or forecasts from hand-typed numbers when a Jira snapshot is one
  MCP call away â€” bridge real data
- âŒ Close a loop with unverified tasks, or report an exhausted budget as success
- âŒ Let an agent be the assignee of record â€” humans own, agents contribute
- âŒ Auto-transition Jira issues or touch permissions inside a loop without the named
  approver

## References

- [references/flow_forecasting_canon.md](references/flow_forecasting_canon.md) â€” Kanban
  Guide 2025, Vacanti Monte Carlo, DORA 2025, EBM, SPACE
- [references/agentic_delivery_governance.md](references/agentic_delivery_governance.md) â€”
  Linear/Rovo delegation models, Anthropic agent patterns, audit discipline
- [references/pm_loop_playbook.md](references/pm_loop_playbook.md) â€” the five reusable PM
...
- Loop engine: `engineering/agent-harness` Â· Loop vocabulary: `loop-library`
