...
|---|---|---|
| Codex / OpenAI Agents | `~/.agents/skills/` | **Canonical** â€” author skills here first |
| Claude Code | `~/.claude/skills/` | **Symlink â†’ `~/.agents/skills/`** â€” writing to `.agents/skills` automatically covers Claude |
| Pi Agent | `~/.pi/agent/skills/` | **Symlink â†’ `~/.agents/skills/`** â€” auto-covered. (Path is `/agent/` nested â€” NOT `~/.pi/skills/`) |
| Hermes Agent | `~/.hermes/skills/` | Independent copy â€” the only one needing a manual copy |
...
2. **Verify the `.claude` symlink is intact** (one-time check):
   ...
   ```
   If it's a real directory instead of a symlink, the user has diverged copies â€” ask before touching.
3. **Copy to `.hermes` only** (`.claude` and `.pi` are symlinks â€” already covered):
   ```bash
   SKILL=<skill-name>
...
4. **Verify all 4 locations** show identical byte counts:
   ...
   ```
   All four numbers must match. If `.claude` or `.pi` shows a different byte count, that symlink is broken â€” investigate before proceeding.

## Updating an Existing Distributed Skill

Same flow â€” re-copy from `~/.agents/skills/` to `.hermes/skills/`. The `.claude` and `.pi` symlinks update automatically. `cp -r` overwrites by default; use `rsync -a --delete` if the skill folder has nested files that may have been removed:
...
- **`~/.pi/skills/` is the wrong location.** Pi Agent loads from `~/.pi/agent/skills/` only. A skill placed in `~/.pi/skills/` is invisible. If you find skills already there, they're orphans â€” confirm with the user before deleting.
...
- **Project-local skills exist too** â€” `./.pi/agent/skills/` (or `.pi/skills/`) inside a repo overrides the global one on collision (later-discovered wins). This skill only handles GLOBAL distribution.
- **`.pi/agent/skills` is a symlink â†’ `.agents/skills`.** Don't `cp` into it (errors "are identical"); it auto-syncs. Only `.hermes/skills` is an independent copy â€” don't unilaterally consolidate Hermes into a symlink unless the user asks.
...
- Skill is project-specific â†’ put it in `./.claude/skills/`, `./.pi/agent/skills/`, etc. inside the repo, not globally.
- Editing one agent's skill only (e.g. a Hermes-only workflow) â†’ patch that file directly, don't propagate.
