...
`/goal` is a slash command that turns an agent prompt into a **persistent agent** looping `plan â†’ act â†’ test â†’ review â†’ iterate` until a stop condition is met, the user pauses, or the token budget runs out. Internally called the "Ralph loop."
...
- An agent with the `/goal` feature â€” right now: Codex, Claude Code, or Hermes Agent
- The goals feature enabled in the agent's config
- **Subscription auth** â€” API-key auth does **not** work. A pro-tier plan is the realistic minimum for long runs.

## When to Use it

Use only when **all three** are true:
1. Task is >30 min of mechanical work.
2. There's a **verifiable stop condition** (tests pass, coverage hit, eval â‰¥ X, build green).
...
1. **Objective** â€” one sentence, one concrete outcome.
2. **Constraints** â€” what must NOT change (public API, files, libs, conventions).
3. **Validation command** â€” the exact shell command that proves progress (`pytest -q`, `pnpm test`, etc.).
4. **Stop condition** â€” verifiable: "Stop when X passes" OR "when further changes need human/product input."
5. **Documentation** â€” one sentence instructing the agent to write concise, targeted docs for every change, either creating new `.md` files or updating existing ones.
...
When the user wants a quick `/goal` instruction, produce a structured markdown block with one line per contract item (proper newlines, not flowing prose). **Do not prefix the output with `/goal`** â€” the user adds the slash command themselves in the composer. Emit only the contract body. Template:
...
**Document:** Write concise, targeted documentation for all changes â€” create new `.md` files or update existing docs as needed.
...
```
**Objective:** Raise coverage in src/auth/ from ~38% to â‰¥75%.
...
**Stop when:** coverage â‰¥75% AND all tests pass, OR when uncovered code needs design changes
```

### Writing rules
- **One objective, one stop condition.** Not a backlog.
- **Documentation is mandatory.** Every `/goal` prompt must include a single sentence committing the agent to concise, targeted docs â€” new `.md` files or focused updates to existing docs.
- **Never instruct the agent to create new ADRs** â€” ADRs require the user's explicit approval, so goal prompts must not pre-approve or encourage them.
...
- **4,000-char limit** on the objective. If longer, put detail in a file (`PLAN.md`/`GOAL_BRIEF.md`) and make the goal point to it â€” keep the goal itself compact.
- Use **literal strings** for paths, commands, issue numbers â€” exact.
...
Budget-limited state: the agent doesn't stop abruptly â€” it summarizes, notes what's left, saves state. `/goal resume` works after budget refresh or upgrade.

## When a goal drifts

- **Minor drift:** just type a correction in the composer (auto-pauses, folds it in, resumes).
- **Loose objective:** `/goal pause`, read status, then `/goal <tighter version>` â€” replaces the contract. Don't pile instructions on a vague goal.
...
- **Always review the diff** before merging â€” long autonomy means more code to validate, not less. Human oversight becomes more critical, not optional.
...
| Stopped with progress summary | Budget-limited â€” `/goal resume` after refresh, or tighten scope |
| `/goal resume` says no active goal | Terminal state or cleared â€” start fresh with `/goal <new>` |
| Goal looks active but won't auto-continue | Stuck in Plan mode â€” plan-only work doesn't trigger continuation. Draft the plan, then switch to Goal execution |
