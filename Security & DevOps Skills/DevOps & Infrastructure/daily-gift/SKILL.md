---
name: daily-gift
description: "Relationship-aware daily gift engine with five-stage creative pipeline â€” editorial judgment, synthesis, concept generation, visual strategy, and rendering in H5, image, or video"
...
Extract six content slots from conversation context (today_theme, emotion_peaks, historical_echo, open_loop, lobster_judgment, preference_hint). Form a gift thesis = anchor (which moment deserves the center) + return (what new perspective the agent gives back). If the thesis has no return, it's not a gift â€” it's a decorated log entry.
...
- **Multi-layer anti-repetition**: concept family, visual elements, theme, style, content direction â€” each tracked across sliding windows of recent gifts
- **Three-layer user taste profile**: Layer 1 (identity â€” stable), Layer 2 (context â€” updates every 5-7 gifts), Layer 3 (signals â€” auto-appended after every gift)
...
- âœ… Let the editorial judgment decide â€” not every day needs a gift
- âœ… Generate 5+ concept candidates before selecting one
- âœ… Check recent gifts for visual and thematic collision before rendering
- âœ… Use the taste profile to personalize over time
- âŒ Don't skip straight from thesis to rendering without a real creative concept
- âŒ Don't default to "reflect on today" every time â€” vary content direction
- âŒ Don't choose the format before locking the concept

## Limitations

- Requires API keys for image/video generation (optional â€” H5 works without them)
...
- The skill reads conversation context and memory files to inform editorial judgment â€” this is core to personalization but means it has broad read access within the agent's workspace.

## Related Skills

- Image generation skills â€” for standalone image creation without the gift pipeline
- Cron/scheduling skills â€” for understanding the daily trigger mechanism
