...
Maps a concrete change (code diff, architecture design, IaC, pipeline config) to the specific controls it touches in financial services compliance frameworks â€” PCI-DSS v4.0 for payment card data and MAS TRM for Singapore-regulated institutions â€” and reports gaps with actionable remediation. This is engineering-level compliance triage: it helps teams catch violations before audit, but it does not replace a qualified assessor (QSA) or the institution's compliance function. Say so in every report.
...
Select the applicable controls from the loaded reference file(s) â€” typically 5-15 controls, not the whole framework. List what you ruled out and why (one line each) so the scoping is auditable. Assess each as `Compliant` / `Gap` / `Needs evidence` (can't tell from the artifact â€” name the evidence required).
...
**Frameworks:** [PCI-DSS v4.0 / MAS TRM 2021] Â· **Date:** [YYYY-MM-DD]
**Scope:** [what was reviewed: files, design doc, pipeline]
> Engineering triage only â€” not a substitute for QSA assessment or the compliance function.
...
- [Control area] â€” [one-line reason]
...
**Skill**: Loads pci-dss.md â†’ Critical findings against 3.3.1 (CVV must never be stored post-authorization â€” logs are storage), 3.4.1 (PAN display masking), 3.5.1 (PAN unreadable at rest); remediation: remove the log line or apply a field-allowlist redaction filter; flags downstream log-pipeline scoping (10.3.x); QSA disclaimer included.
...
**Skill**: Loads mas-trm.md â†’ reviews against Â§11.5 (cloud: due diligence, data residency, exit strategy), flags the MAS Outsourcing Guidelines as a related instrument, asks what customer data the service touches before rating severity.

## Common FSI Engineering Triggers

Changes that almost always have compliance impact â€” check proactively when they appear in a diff:

- Logging statements near payment or authentication flows (PAN/CVV must never be logged; MAS TRM requires security event logging â€” both directions matter)
...
- When the change is compliant, say so affirmatively per control â€” "no findings" plus the checked-control list is a useful audit artifact.
