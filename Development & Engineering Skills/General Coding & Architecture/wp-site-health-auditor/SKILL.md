...
prioritized, risk-tiered fix plan â€” then executes the safe fixes and hands off the rest with exact
commands or code.

## âš ï¸ Safety â€” read before touching any file
...
3. **Never run `wp search-replace` without `--dry-run` first**, and always show the dry-run output to the
   user before running it for real. This command rewrites the database in place â€” a wrong pattern can
...
5. **Change one thing at a time, then verify the site still loads** (homepage + wp-admin) before making
   the next change. Don't batch multiple Tier 2 file edits into one pass â€” if something breaks, you want to
...
6. **Give the user the exact rollback command** alongside every edit:
   ...
   ```
   State this even if nothing goes wrong â€” it costs one line and saves a panicked user later.

If the user says "just do it, skip the backup" â€” still create the backup silently as part of the edit
...
backend) or are purely informational (SQL server version â€” no action needed). This skill sorts that out â€”
safely.

## Phase 1 â€” Parse the report
...
- WP-CLI output (`wp site-health check` is not a core command; note that up front, don't invent one â€” see Phase 4)

Extract three buckets exactly as WordPress labels them:
1. **Critical issues** (red) â€” always fix first, always confirm before touching.
2. **Recommended improvements** (yellow) â€” the bulk of real work; triage by risk tier below.
3. **Passed tests** (green) â€” skip. Do not "fix" or re-verify passed tests unless the user asks. Do not
   invent problems with green items â€” a common failure mode is treating "SQL server is up to date" as
...
verbatim before triaging â€” don't paraphrase the WordPress-generated title, it's used for the fix lookup in
Phase 3.

If no report was pasted and the user just says "audit my site health," ask them to paste the Status tab
text (fastest) rather than guessing â€” Site Health results are host- and config-specific and guessing wastes
a turn.

## Phase 2 â€” Risk-tiered triage

Classify every non-passed item into one of three tiers before touching anything. Present this triage table
to the user first for anything above Tier 1 count of 3+ items â€” don't silently start deactivating plugins.

**Tier 1 â€” Safe, reversible, auto-fixable in wp-admin or via WP-CLI**
...
- Remove inactive plugins/themes (they aren't running, deactivation already happened â€” this is just
...
**Tier 2 â€” Requires host/server-level access â€” Claude drafts the change, user or host applies it**
...
- Permalink structure change (migration â€” existing URLs break without redirects; require a redirect plan and CDN/cache flush before applying)
...
**Tier 3 â€” Informational / host-dependent, no fix exists or none needed**
...
match anything here, say so explicitly rather than fabricating a fix â€” Site Health's item set changes
...
### You should remove inactive plugins / themes â€” Tier 1
```
...
Twenty Twenty-Five (or the current default core theme) if it's the only fallback theme â€” WordPress needs
at least one broken-theme fallback; recommend keeping one bundled default even if inactive.
Confirm the exact plugin/theme names with the user before deleting â€” inactive isn't the same as unused;
some plugins are intentionally kept inactive as a staged rollback.

### post_max_size smaller than upload_max_filesize â€” Tier 2
...
1. Host control panel PHP settings (cPanel "Select PHP Version" > Options, Plesk, etc.) â€” no code needed,
   safest option, skip the file-backup steps entirely.
2. `php.ini` (if the user has server access) â€” back up first (`cp php.ini php.ini.bak-<timestamp>`):
   ```ini
   upload_max_filesize = 64M
   post_max_size = 128M
   ```
3. `.htaccess` (Apache + mod_php only, not on PHP-FPM/nginx) â€” back up first:
   ```apache
...
4. `.user.ini` (CGI/FastCGI hosts; not mod_php) â€” back up first, create or edit `.user.ini`
   in the WordPress root:
   ```ini
   upload_max_filesize = 64M
   post_max_size = 128M
   ```
   âš ï¸ **Do not use `ini_set()` in `wp-config.php` for these directives** â€” `upload_max_filesize`
...
(`wp cli info` doesn't show these â€” check `phpinfo()` or the host panel) rather than assuming defaults.

### You should use a persistent object cache â€” Tier 2
Requires a caching backend (Redis or Memcached) installed at the server level â€” this is not something a
...
2. If available, install a drop-in client plugin: Redis Object Cache or WP Redis (Redis), or Memcached
   ...
   an `object-cache.php` drop-in to `wp-content/` â€” confirm no existing `object-cache.php` is being
   overwritten (check first with `ls wp-content/object-cache.php`); if one exists, back it up before enabling.
3. If not available, this is a hosting-tier limitation â€” report it as such rather than trying to fake a
   fix; don't recommend switching hosts unprompted, just flag it as the blocker.

### Page cache is not detected â€” Tier 2
1. Check if the host provides server-level page caching (many managed WP hosts do, and it may already be
   active but not reporting the headers Site Health looks for â€” worth confirming with the host before
   installing a redundant plugin).
2. If not, install one page-cache plugin (not a full plugin stack) â€” WP Super Cache, W3 Total Cache, or
   the host-recommended one. `wp plugin install wp-super-cache --activate` then enable caching from its
   settings screen (no reliable WP-CLI toggle across cache plugins â€” flag manual step to user).
3. Avoid stacking two caching plugins; if one is already active but not detected, check the plugin's own
   status page before adding another. Some cache plugins also write rules into `.htaccess` â€” back it up
   first per the Safety section before activating.

### Your site is not set to output debug information â€” usually already passing; if failing â€” Tier 1
...
### REST API / loopback requests / background updates failing â€” Tier 2
...
### HTTPS not fully active â€” Tier 2
```
...
## Phase 4 â€” What NOT to invent

- There is no `wp site-health` WP-CLI command in WordPress core as of this writing â€” don't fabricate one.
  Fixes are applied via the specific commands above, not a single audit-and-fix CLI call.
- Don't claim a fix is complete without the user (or a re-run of Site Health) confirming it â€” server-level
  changes (Tier 2) especially can silently fail to apply depending on host restrictions.
- Don't guess PHP/server values (current `upload_max_filesize`, cache backend availability, etc.) â€” ask or
  have the user check `phpinfo()` / host panel rather than assuming common defaults are in place.
- Don't skip or shortcut the Safety section for any reason, including "it's a small change" â€” file
...
Keep the whole response scannable â€” this is a punch list, not an essay. Use the table + short recipe
...
1. Triage â†’ Tier 2 (requires Redis/Memcached at server level)
...
1. Triage â†’ Tier 1 (safe, reversible via wp-config.php)
...
- âœ… Back up the specific file before every edit â€” `cp` takes seconds, restoring a dead site takes hours
- âœ… Change one thing at a time and verify the site loads between each change
- âœ… Always run `php -l` after editing `wp-config.php` before reloading the site
- âœ… Run `wp search-replace` with `--dry-run` first and show the output to the user
- âŒ Never batch multiple Tier-2 file edits into one pass â€” you won't know which change broke the site
- âŒ Never skip the backup step, even for a one-line comment change

## Reference

`references/catalog.md` â€” extended list of less-common Site Health items (SEO category items like llms.txt
...
- **Treating every yellow item as actionable** â€” Some recommended improvements (e.g. persistent object cache) are host-level and may not be fixable. Always triage by tier before acting.
- **Changing permalinks without a redirect plan** â€” Flipping to "Post name" on an indexed site breaks every existing URL. Always plan 301 redirects first.
- **Using `ini_set()` for upload limits** â€” `upload_max_filesize` and `post_max_size` are `PHP_INI_PERDIR`; `ini_set()` silently fails. Use `php.ini`, `.htaccess`, or `.user.ini` instead.
- **Skipping the dry-run on `wp search-replace`** â€” A wrong pattern can corrupt serialized data. Never run it without `--dry-run` first.
- **Installing two caching plugins** â€” Stacking page cache plugins causes conflicts and obscure bugs. If one is already active but not detected, debug it rather than adding another.

## Limitations

- Cannot execute anything itself against a live site â€” every WP-CLI/PHP snippet is drafted for the user or
  their host to run; this skill has no shell access to the user's actual server.
- Cannot verify current PHP/server values (upload limits, cache backend availability, HTTPS status) â€”
...
- The item catalog (main file + `references/catalog.md`) reflects WordPress core's Site Health checks as of
  mid-2026 â€” item titles/wording can change across core versions, so an unmatched item should be reported
  as unmatched, not force-fit to the closest recipe.
- Does not replace a full security audit or compromise scan â€” Site Health flags configuration hygiene
  issues, not signs that a site has already been broken into.

## Related Skills

- `@security-hardening` â€” For deeper WordPress security audits beyond Site Health's surface checks
- `@wp-performance` â€” For targeted performance optimization after Site Health flags are resolved
