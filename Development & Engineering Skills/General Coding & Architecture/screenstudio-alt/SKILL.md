...
description: "Open-source headless Screen Studio alternative: auto speed-up of idle, auto-zoom on click clusters, keystroke overlay chips, smoothed synthetic cursor, and 9:16 vertical export that follows the action â€” post-production for screen recordings from the CLI."
risk: critical
...
- `events-log` (Swift) â€” capture-side input logger (cursor 60Hz, clicks, keys;
  ...
  this data at capture time â€” it cannot be recovered from pixels later.**
- `polish.py` (Python, ffmpeg + PIL) â€” the post-production pass:

```bash
python3 src/polish.py in.mp4 --events in.events.jsonl \
  --speedup            # compress idle (input-gap âˆ© frozen-pixels; animations stay 1x)
...
`--speedup` works WITHOUT events (freezedetect only) â€” usable on the whole
existing dailies corpus.

- `render.py` â€” **high-quality non-destructive renderer** (preferred): single-pass
  spring-physics camera over the original high-res frames, LANCZOS into a smaller
  target (crisp zoom, ~1.3Ã— sharper than the ffmpeg upscale path), 60fps, H + 9:16 V.
  Tunable `--freq`/`--zeta` (spring), `--fps`, `--target-w`. Takes explicit
  `--regions [{t0,t1,z,cx,cy}]`. `polish.py` is the older ffmpeg-filter fallback.
- `studio.py [recording.mp4]` â€” local web UI, **NLE-style fixed-ruler timeline** (bar =
  source duration, never rescales â†’ upstream always planted): zoom regions are draggable
  blocks (move / retime edges / click to add / double-click delete); idle spans are
  **speed blocks with rate-only editing** â€” source range locked, rate set via inspector
...
- ffmpeg CANNOT do animated `scale=eval=frame` â†’ `crop` (link reinit wedges crop's
  per-frame exprs). That's why zoom uses `zoompan` (no `t` var there â€” use `on/FPS`).
...
- Test rig: `make-fixture.py` synthesizes a fake screen recording + ground-truth
  events.jsonl â€” validate any change against it before trusting real footage.
