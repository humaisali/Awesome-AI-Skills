...
description: "macOS screen recorder that captures the main display PLUS system audio via ScreenCaptureKit â€” no BlackHole/loopback driver, no sudo, just the standard Screen Recording permission. CLI-driven; fills the headless-screen-recording-with-system-sound gap QuickTime and `screencapture -v` can't."
risk: critical
...
Use when you need to script a screen recording WITH system sound on macOS from the CLI (demos, captures, voice-demo recording) â€” the case QuickTime and `screencapture -v` can't cover without a virtual audio device.
...
`sck-record.swift` â†’ compiled `sck-record` (binary gitignored; built by `setup-machine`, or
...
**The one true differentiator:** system audio from the CLI with **zero install** â€” no
...
Studio" tool â€” it fills exactly the headless-CLI-with-system-audio gap.

`sck-record` is the raw capture primitive â€” it records, nothing more. To polish a
