...
# lore â€” Framework-agnostic Memory Management
...
**Tier 1 â€” Loading the skill.** Load this skill when the user explicitly invokes `lore`, names a subcommand, references `.lore/`, or asks to record, recall, audit, sync, or compress project memory about decisions, architecture, conventions, or monorepo scopes. Generic phrases like "init", "compress", "audit", or "query" alone are not enough â€” they may map to the agent's native commands or unrelated tasks (Claude Code's `/init`, `/compact`, security audits, SQL queries, etc.).
...
**Tier 2 â€” Internal proposals (after the skill is loaded).** Once the skill is loaded for this session, certain commands may proactively propose themselves based on internal thresholds. These proposals still require user acceptance â€” the skill never mutates files silently.

- `sync` proposes when â‰¥50 changed lines span â‰¥2 directories, OR a new top-level module/directory/dependency was added or removed, OR a new convention was explicitly discussed in chat.
...
Other commands (`init`, `query`, `history`) are always explicit â€” they need user intent. See [`WORKFLOWS.md`](WORKFLOWS.md) for a plain-language explanation of when each workflow is used.
...
| `references/summary-template.md` | Running `compress` â€” SUMMARY.md schema and selection rules |
| `references/audit-template.md` | Running `audit` â€” report format and severity definitions |
| `references/monorepo-detection.md` | During `init` â€” detecting scope boundaries from workspace config |
| `references/stale-new-markers.md` | During `sync` â€” full marking convention and user reply semantics |
...
| `references/history-command.md` | Running `history` â€” full spec, dispatch rules, error table |
| `references/compatibility.md` | Versioning policy: `.config.json#schema_version`, migration tools, deprecation workflow |
| `scripts/README.md` | Helper scripts (id_hash, list_entries, find_duplicates, find_stale) â€” also in Chinese (`scripts/README.zh-CN.md`) |
...
```
.lore/
â”œâ”€â”€ SUMMARY.md              # Top-level digest. New agents read this first.
â”œâ”€â”€ _global/                # Cross-scope facts (whole-project architecture, global decisions)
â”‚   â”œâ”€â”€ ARCHITECTURE.md
â”‚   â”œâ”€â”€ DECISIONS.md
â”‚   â””â”€â”€ CONVENTIONS.md
â”œâ”€â”€ scopes/                 # Per-scope facts
â”‚   â”œâ”€â”€ <scope-name>/
â”‚   â”‚   â”œâ”€â”€ ARCHITECTURE.md
â”‚   â”‚   â”œâ”€â”€ DECISIONS.md
â”‚   â”‚   â””â”€â”€ CONVENTIONS.md
â”‚   â””â”€â”€ ...
â”œâ”€â”€ draft/                  # Used only by `init`. Proposals pending user confirmation.
â”œâ”€â”€ audit/                  # Used only by `audit`. Reports; never mutates main files.
â””â”€â”€ archive/                # Old/superseded entries, kept for history
```
...
**Decisions placement:**
- Affects â‰¥ 2 scopes (e.g. "use pnpm workspaces", "TypeScript strict") â†’ `_global/DECISIONS.md`
- Affects exactly one scope â†’ that scope's `DECISIONS.md`

There is no separate metadata file. Every status lives as inline tags on entries themselves.

### Entry format

Each entry is a Markdown bullet (â‰¤ 2 lines), with a layer prefix, a deterministic ID, and inline status tags. See `references/entry-format.md` for the full spec (ID generation via content hash, tag semantics, cross-file reference format, splitting rules).
...
By default the Lore section is an **index** into `.lore/` â€” paths plus a per-scope one-line description, ~500 bytes total. The agent reads `.lore/SUMMARY.md` (or calls `lore query <term>`) on demand. See `references/platform-mirrors.md` for the template and adaptive rendering rules.
...
LangGraph / DeepAgents typically don't need a mirror file â€” they read `.lore/*.md` directly or ingest into the system prompt at runtime (the user's responsibility).
...
|---|---|---|
| Claude Code `/init` | One-shot project scan â†’ generates `CLAUDE.md` | `lore init` (creates `.lore/` + mirror files) |
...
- If both `lore sync` and `/compact` are available, they do unrelated work â€” run them independently.
...
- **Record a decision:** User says `lore sync â€” we picked Zustand because Redux boilerplate was slowing down onboarding`. The skill appends `[DEC-YYYY-MM-DD-XXXX]` to the active scope's `DECISIONS.md` and proposes the change for confirmation.
...
### `init` â€” Initialize the memory bank

Runs once per project (or to start over).

0. **Resolve targets and takeover check.** Targets are determined by the resolution algorithm â€” see `references/platform-mirrors.md`. Default behavior: scan repo root for existing platform files; if none found, ask the user via multi-select which agents they use. Explicit `mirror_targets` in `.lore/.config.json` overrides auto-detect (Replace semantics), but never bypasses validation. Validate the complete list against the platform allowlist and canonical project-root containment rules before reading or writing any target; reject absolute paths, `..`, unsupported paths, and symlink escapes atomically. For each validated target:
   - If the file does not exist â†’ no action; it will be created later in step 7.
   - If the file exists AND contains a `## Lore` section â†’ it's already a lore mirror; note it and continue (its My notes will be processed as seed in step 5).
   - If the file exists AND does NOT contain a `## Lore` section â†’ it's likely from the agent's native `/init` or hand-written. Show the user:
     - (a) **Take over** â€” rewrite the file as a two-section mirror. The existing content becomes the My notes section (preserved verbatim, treated as seed knowledge in step 5).
     - (b) **Preserve as-is** â€” leave the file alone. Remove it from `mirror_targets` for this project (lore won't write to it). `.lore/` is still generated normally; the user can read `SUMMARY.md` directly or merge manually later.
     - (c) **Abort** â€” exit init. Nothing is created. The user can decide later.
...
2. Detect monorepo structure (per `references/monorepo-detection.md`). Propose scope list to the user; let them rename / merge / split before proceeding. No monorepo â†’ `_global/` only.
...
6. **Stop and show the user a summary**: which scopes, how many entries per layer per scope, sample of 5â€“10 entries, and what mirror files will be (re)generated (or skipped per step 0).
7. On user confirmation: `mv .lore/draft/* .lore/`, run an initial `compress` to generate `SUMMARY.md`, then (re)generate platform mirrors per the two-section structure â€” auto-create missing files, refresh Lore sections, leave My notes sections intact. Skip any target the user chose "preserve as-is" in step 0.
...
### `sync` â€” Update after a change

Runs after the user completes a feature, refactor, or bug fix.

**Trigger threshold â€” only propose sync when at least one is true:**
- `git diff --stat HEAD` shows â‰¥ 50 changed lines across â‰¥ 2 directories
...
If any of these are true, the skill appends a `[COMPRESS NOTICE]` to the sync proposal. It does not block the sync â€” the user can defer.
...
1. **Detect the delta** from two sources, combined and de-duplicated:
   ...
   - `git diff` (working tree vs. `HEAD`) â€” always included. Catches uncommitted changes that are not yet in any commit.
   - **Re-scan any new files**.
   - **Fallback** when `last_sync_sha` is absent (older config) or no longer reachable (e.g. after `git rebase` or a force-push that orphaned the SHA): use `git diff HEAD` alone and emit a one-line `[WARN]` to stderr noting that incremental sync is degraded. Working tree alone will not pick up commits made before the next sync ran â€” the user should re-run `sync` after `git pull --rebase` to re-establish the baseline.
   - **Empty repo** (no commits yet): `last_sync_sha` is `null`; only the working tree diff applies.
2. **Determine target scope(s)** for each change. Use `git diff --name-only` paths (over the combined commit + working-tree diff) to map files â†’ scopes (e.g. `frontend/src/...` â†’ `scopes/frontend/`). Cross-scope changes (root config files) â†’ `_global/`.
3. **Classify each change** into one layer:
   - New module, new dependency, new file structure â†’ `ARCHITECTURE.md`
   - "We picked X over Y because Z" â†’ `DECISIONS.md`
   - New lint rule, new naming pattern, new "we never do X" â†’ `CONVENTIONS.md`
4. **For each candidate entry**:
   - **Contradicts an existing entry** in the same scope/layer â†’ mark the old one `#stale:<today>`. Emit an `ALERT`.
   - **Refines an existing entry** â†’ update the text in place, bump `#verified:<today>`.
   - **Genuinely new** â†’ append with `#added:<today>` and a new hash ID.
5. **De-duplicate**: before appending, run `python scripts/find_duplicates.py --json` to identify any candidate entry that overlaps with existing entries (same hash, or Jaccard â‰¥ `--threshold`). For each match, skip the new entry and bump `#verified` on the existing one. If the new entry is genuinely different in meaning (the script flags but doesn't decide), keep both.
...
9. After the user accepts, write to `.lore/*` only. **Do not** regenerate platform mirrors from `sync` â€” this is intentional. See "Mirror update triggers" below for the rationale and the dedicated `lore mirror` command.
10. **Update `.lore/.config.json#last_sync_sha`** to the current `git rev-parse HEAD`. Idempotent: re-running sync without new commits writes the same SHA. If HEAD does not exist (empty repo), set to `null`. The bump from v1 â†’ v2 added this field; v1 configs without it keep working through the fallback in step 1.

**Source priority** (when sources disagree):

1. Git diff of changed code (most reliable â€” shows what actually happened)
2. Static scan of new files (reliable for facts, not for intent)
3. Conversation context (lowest priority â€” see below)
4. Test/build output (auxiliary â€” only consulted if 1â€“3 are ambiguous)
...
1. `init` completion â€” first time the mirror is created or restructured
2. `compress` completion â€” `SUMMARY.md` changed, so mirrors reflect the new digest
3. Explicit `lore mirror` command â€” user forces a regeneration
...
### `mirror` â€” Regenerate platform mirrors
...
### `query` â€” Answer from memory

Read-only.

1. Determine which scope(s) the question targets:
   - "this project" / "the whole codebase" / unspecified â†’ `_global/` first, then SUMMARY.md
   - "frontend" / "in the web app" / "the React side" â†’ `scopes/frontend/`
   - "backend" / "the API" â†’ `scopes/backend/`
...
### `history` â€” Show git commits related to a memory entry
...
does not
trigger â€” defer to the user's intent.
...
6. Resolve the code file: backtick path in entry text â†’ scope
...
### `audit` â€” Check memory vs. reality
...
### `compress` â€” Build the top-level summary
...
3. For each (scope, layer) pair, pick 3â€“5 most important entries using the selection rule in `references/summary-template.md`.
...
**Typical sequence:** `init` â†’ `[sync â‡„ query â‡„ audit]` (interchangeable, agent picks by context) â†’ `compress` (when SUMMARY.md grows stale) â†’ `mirror` (or auto via `compress` if `auto_mirror: true`).
...
- **Do let `audit` run on its own schedule.** Don't skip audits because the project feels "obviously fine" â€” the staleness check exists precisely for the cases you don't notice.
...
- **Don't trigger on the agent's native `/init` or `/compact` calls.** lore only fires when the user explicitly says `lore <command>`. Bare "init" / "compress" / "initialize" is the agent's native command â€” defer to it. If the user later wants to integrate a native-init `CLAUDE.md` with lore, point them at `lore init` step 0.
...
```
lore init      # Step 0 takeover check â†’ scan â†’ draft into .lore/draft/ â†’ user confirms â†’ move to .lore/.
