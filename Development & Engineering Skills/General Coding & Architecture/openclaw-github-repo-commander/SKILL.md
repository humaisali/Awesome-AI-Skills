...
A structured 7-stage super workflow for comprehensive GitHub repository management. This skill automates repository auditing, cleanup, competitor benchmarking, and optimization â€” turning a messy repo into a clean, well-documented, production-ready project.
...
### Stage 2: Execution
Run `scripts/repo-audit.sh <repo-path>` from this skill directory â€” automated read-only checks for:
...
```
Clean up my GitHub repo â€” remove junk files, fix secrets, add .gitignore
```
...
- âœ… Always run Stage 7 validation before pushing
- âœ… Use semantic commit messages: `chore:`, `fix:`, `docs:`
- âœ… Check the `pr_todo.json` file for pending reviewer requests
- âŒ Don't skip Stage 4 â€” competitor analysis reveals blind spots
- âŒ Don't commit `node_modules/` or `.env` files
...
- All `gh` CLI operations use the user's existing authentication â€” no credentials are stored by this skill
