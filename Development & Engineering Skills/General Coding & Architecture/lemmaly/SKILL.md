...
# lemmaly â€” Algorithm-First Proof
...
**Violating the letter of these rules is violating the spirit of the skill.** "Just this once" is how O(nÂ²) ships to production.
...
- Reviewing AI-generated code that "looks idiomatic" but might hide O(nÂ²) or N+1.

When in doubt, **start at lemmaly** â€” it is the gateway and will tell you when to escalate to its three sibling skills.

| If you are about toâ€¦ | Use | Why |
| --- | --- | --- |
...
Before you write a loop, a recursion, a query, or any computation over more than a handful of items, three things must appear in your message â€” in this order:
...
2. **Name the data structure with a one-phrase reason.** Every collection-shaped value gets a deliberate choice from `Array / List / Set / HashMap / TreeMap / Heap / Deque / Trie / Graph / BitSet / Counter / LinkedList` â€” with the reason: "Set for O(1) membership inside the loop", "Heap for top-K in O(n log k)", "Counter to fold the nested loop into a single pass". Default to hashed structures (`Set`, `Map`) for lookup inside loops. Default to streaming/iterator over materialized list when n is large.
...
4. **Repeated work in loops is algorithmic waste.** All of these are presumed wrong until justified:
   - I/O inside a loop (database queries, HTTP calls, file reads) â€” batch with `IN (...)`, `Promise.all`, bulk endpoints, streaming
   - Recomputing the same value in a loop â€” hoist or memoize
   - Re-sorting / re-grouping inside a loop â€” sort once outside
   - Linear scan (`.find`, `.indexOf`, `.includes`, `in list`) inside a loop â€” precompute an index `Map`
   - Allocating fresh structures per iteration when one can be reused â€” hoist allocation
   - Materializing intermediate collections only to iterate again â€” fuse into one pass
...
Before producing non-trivial code, your message must contain â€” in this order:

1. **Problem shape** â€” one sentence. ("Given n events with a timestamp, find the longest contiguous window where total weight â‰¤ K.")
2. **Input dimensions** â€” `n = ?`, realistic magnitude, whether hot path.
3. **Target complexity** â€” `time = O(?)`, `space = O(?)`.
4. **Data structures** â€” name them with a phrase each.
5. **Algorithm family** â€” one phrase.
6. **Edge cases you will handle** â€” empty, singleton, all-equal, n=1, n=max, overflow, duplicates. List the ones that apply.
7. **The code.**

If any of 1â€“6 is missing, do not emit code yet.

## Canonical example â€” protocol vs no-protocol
...
### Without the protocol â€” ships O(nÂ·m)

```ts
// Looks idiomatic, ships O(nÂ·m)
const active = users.filter((u) => !bannedIds.includes(u.id));
```

`bannedIds.includes` is O(m) per call. The filter runs it n times â†’ 50k Ã— 5k = 250M comparisons.

### With the protocol â€” O(n + m)
...
//   edge cases: empty users â†’ [], empty bannedIds â†’ users, duplicates in bannedIds â†’ fine (Set dedupes)
const banned = new Set(bannedIds);
const active = users.filter((u) => !banned.has(u.id));
```

The first version is the default an AI ships when asked "filter the active users." The second is what the protocol forces â€” without changing how the code reads.
...
- `js-await-in-for-loop` â€” N+1 over network
- `js-async-in-foreach` â€” dropped promises
- `py-mutable-default-arg` â€” shared default state
- `sql-update-no-where` â€” touches every row
- `java-arraylist-remove-in-for-i` â€” index shifts; ConcurrentModification
- `cs-async-void` â€” exceptions unobserved; crashes the process
- `go-loop-var-capture` â€” pre-1.22 race on the last value
- `php-query-in-loop` â€” N+1 against the database
...
- **Math-level optimization** (probabilistic data structures, FFT, dimensionality reduction, approximation algorithms, computational geometry) â€” load **mathguard**.
- **Algorithm correctness** (loop invariants, termination, recursion base cases, edge cases that tests miss) â€” load **invariant-guard**.
- **Existing code with bad complexity that already shipped** â€” load **complexity-cuts** for the corrective transformation playbook.

## Rationalizations to watch for

These are real verbatim thoughts captured from controlled tests where the model shipped O(nÂ·m) code that the seven-step protocol would have prevented:

| Excuse | Reality |
| --- | --- |
| "`.filter` then `.reduce` is the idiomatic way, ship it." | Idiomatic â‰  correct asymptotic. Idiom-driven coding is how O(nÂ²) ships. |
...
| "The data is small in dev â€” I'll worry about scale when we ship." | Production data is never the size of dev data. The seven-step protocol takes 30 seconds. |
...
## Red flags â€” STOP and restart the protocol

- About to write a `for` inside a `for` without first stating it is the intended O(nÂ·m).
...
- [ ] Any "fast" / "efficient" / "scales" claims have either a derivation or a measurement â€” `<measured: TBD>` is acceptable; an unsupported claim is not.
...
- **Cannot prevent intentional brute force.** If the author writes a one-line justification ("n â‰¤ 100 in practice; readability matters more"), brute force ships. The skill only requires the justification, not its absence.
...
- `mathguard` â€” escalation for n â‰¥ 10â¶ where classical O(n log n) is the floor and probabilistic / math-heavy techniques win.
- `invariant-guard` â€” correctness layer for algorithms whose obvious version is subtly wrong.
- `complexity-cuts` â€” corrective playbook for code that already shipped with bad Big-O.
