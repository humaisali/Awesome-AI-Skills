...
wgm ("well, gosh... make") is a portable build **methodology**, not a domain skill â€” a single
...
- Not for trivial one-file edits, pure debugging, research-only questions, or tasks that already
  have complete, unambiguous step-by-step instructions â€” wgm explicitly stays out of the way there.

## How It Works

### Step 1: Triage
Classify the work onto a scale-adaptive track (Quick / Standard / Full) so ceremony matches risk â€”
...
success criteria, and constraints are known â€” capping interrogation after ~5 questions to avoid
...
path), holdout acceptance scenarios the build must never read, and `IMPLEMENTATION_PLAN.md` â€” the
...
dimension â€” do not start building on a shaky plan.
...
`IMPLEMENTATION_PLAN.md`, scores Preflight readiness, then loops one task at a time â€” each task's
own test/lint/build command must exit 0 before it's marked done â€” and finally ships with a
docs-audit pass.
...
â€” useful when a human wants to review the plan before any code is touched.

## Best Practices

- Do let the plan be the shared state â€” a fresh agent should be able to resume a build from
...
- Don't skip the alignment interview on ambiguous, multi-week, or security/UX-critical work just to
  move faster â€” misalignment discovered after building is far more expensive.
- Don't treat a high satisfaction score as sufficient on its own â€” a failing deterministic check
  always overrides it.

## Limitations

- wgm is a protocol, not a runtime: it has no daemon, scheduler, or bundled dashboard â€” it expects
...
- Full holdout-scenario judging and the docs-audit swarm add ceremony that a genuinely trivial task
  does not need â€” wgm's own Triage track exists specifically to right-size this, and the skill
...
- **Problem:** Letting the agent peek at holdout scenarios while implementing.
  **Solution:** Scenarios are read only during Validate/Review, never during Implement â€” that's the
