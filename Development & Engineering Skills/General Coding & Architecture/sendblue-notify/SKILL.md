...
description: "Text the user's phone when a long-running task, agent turn, or scheduled job finishes â€” via @sendblue/cli for outbound, optionally wired to a Claude Code Stop hook for automatic fire."
category: automation
risk: critical
...
- Use when the user says "text me when X is done", "ping my phone", "notify me on completion", "let me know when the build/deploy/migration finishes", or "send me an iMessage whenâ€¦".
...
- Do **not** use for short, interactive tasks where the user is watching the terminal â€” the notify is noise.
...
The user's phone number must be a verified contact on the account. On the free plan, the contact has to text the Sendblue number once before outbound sends work â€” confirm with `sendblue contacts` before relying on notify in an unattended workflow.
...
Notify is for **long, unattended work** â€” not chatter. Good triggers:
...
- Every `Stop` event, regardless of duration â€” produces spam, trains the user to ignore.
...
- **One-shot inline send** â€” default for a single ad-hoc task. No config changes.
- **`Stop` hook** â€” opt-in, project-scoped, for sessions the user explicitly wants on automatic notify. Always gate by duration.
- **End-of-`/loop` or `/schedule` ping** â€” append the send to the routine's body.

### Step 3: Compose the notification copy

- **One line, under ~140 chars** â€” fits in the lock-screen preview.
- **Lead with outcome** â€” âœ…/âŒ, "done", "failed", "needs review".
- **Include something actionable** â€” branch name, error tail, PR number, duration.
- **No emojis the user didn't ask for** beyond a single status glyph.
- **No agent self-narration** ("I have completed the task as requested" â€” just say what happened).

## Examples

### Example 1: One-shot inline send

For a single task, append the send to the command. This is the default â€” no config changes, no surprise behavior later.

Branch on the task's exit status with an `if`/`else`. Do **not** use a `task && send-success || send-failure` chain: if the task succeeds but the success-send itself returns non-zero, the `||` fires the failure message â€” so the user sees âŒ even though the task completed. The `if`/`else` keeps the outcome tied solely to the task.

```bash
if long_running_thing; then
  npx @sendblue/cli send +15551234567 "âœ… done: $(date +%H:%M)"
else
  npx @sendblue/cli send +15551234567 "âŒ failed: $(date +%H:%M)"
fi
```
...
npx @sendblue/cli send +15551234567 "migration done â€” $RESULT"
```

### Example 2: Claude Code `Stop` hook (opt-in, scoped)

Register a `Stop` hook in `.claude/settings.json` (project-scoped) â€” never in global settings unless asked. Defer the actual file edit to [[update-config]]. The hook command itself should:

1. Run cheaply (it fires on *every* `Stop`).
2. Gate on duration â€” skip sends for turns under a threshold (e.g. 90s).
3. Never fail the parent â€” pipe to `|| true` so a notify error doesn't surface as a hook failure.
...
(Adjust the env var names to whatever the hook contract actually provides â€” verify against the current Claude Code hooks reference before writing the config; the harness owns those names, not this skill.)
...
If the user has `@textme` installed (njerschow/textme â€” daemon that lets you *text Claude* from your phone), notify is still useful and not redundant. They run in opposite directions:

- **textme**: phone â†’ Claude (user initiates from the phone).
- **sendblue-notify**: local Claude â†’ phone (Claude initiates from a local session).
...
- âœ… **Default to one-shot inline sends** for single tasks. Only escalate to a hook when the user asks for automatic notify.
- âœ… **Gate hooks by duration.** A 90s threshold is a sensible starting default.
- âœ… **Show the hook config before installing it** and get explicit user confirmation.
- âœ… **Store the destination number per-user** (env var or gitignored file), not in committed config.
- âŒ **Don't install global hooks** unless the user explicitly asks. Project-scoped is the default.
- âŒ **Don't let a failed notify fail the parent.** Trail with `|| true`.
...
- **Per-user numbers.** The destination phone number is a personal identifier â€” keep it in user-local config (env var, gitignored file), not in committed repo files or CI logs.
...
- **PII in notification copy.** Lock-screen previews are visible to anyone holding the phone. Don't embed secrets, customer data, or full error stacks â€” link to a log or PR instead.
- **Burying the outcome.** "Task complete. Here is a summary of what I didâ€¦" wastes the preview line. Lead with âœ…/âŒ and the verb.

## Related Skills

- `@sendblue-cli` â€” Owns the actual send mechanism. This skill calls into it.
- `@sendblue-api` â€” HTTP alternative for app code where notify lives inside a long-running service.
- `@update-config` â€” Wires the `Stop` hook into `.claude/settings.json`. This skill owns the *what* and *when*; update-config owns the *where*.
- `@textme` â€” Inbound counterpart (phone â†’ Claude). Composes well with notify.
