...
shared daemon like Docker â€” services run per-user via `launchd`. Images are standard OCI
...
top-level command â€” image verbs live under `container image`).
...
- **Version:** this skill documents the **1.0.0** release (the fullest feature set). The `machine`
  ...
  â€” features that postdate 0.7.1 are flagged *(1.0.0+)* in the reference files. Run `container --version` and
...
2. **Double-click** the downloaded package and follow the prompts, entering your admin
   ...
   invocation â€” installation is via the GUI package.)
...
# offers to install the default Linux kernel â€” accept it, or start non-interactively with
...
`container system start` must have run before any container/image/build command works â€” a
...
`references/commands.md` â€” consult it before running an unfamiliar command rather than
...
- **`references/commands.md`** â€” exhaustive CLI reference: every command group, subcommand,
...
- **`references/concepts.md`** â€” architecture (VM-per-container, Containerization framework),
...
- **`references/configuration.md`** â€” the system service, `config.toml` / property model,
  ...
  kernel.
- **`references/workflows.md`** â€” copy-pasteable task recipes (run an image, build & push,
...
- **Images are standard OCI artifacts** and interoperate with Docker registries and other OCI
  ...
  `registry.domain` property â€” see `references/configuration.md`).
...
- **`container network` requires macOS 26.** On macOS 15 only the single default subnet is
  available and the network command group is unavailable â€” see `references/concepts.md`.
