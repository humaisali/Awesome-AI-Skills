...
Use when a task needs a REAL or ARCHIVAL photo / clip (hero, texture, reference, historical footage) or a reaction / animated GIF, rather than a generated one â€” fan out across free image/video/GIF sources in one query and download license-tagged results.
...
in `central/.env` (optional â€” the 5 no-key sources work without them).
...
| klipy | free key | **GIFs** â€” recommended (free, unlimited, Tenor drop-in) | gif |
| giphy | free key | **GIFs** â€” biggest library (prod key needs approval) | gif |

GIF sources fire only with `--type gif`. Keys: `KLIPY_API_KEY`, `GIPHY_API_KEY`
in `central/.env`. (tenor adapter removed â€” Google EOL'd the API 2026-06-30.)
**klipy** is the one to get (free + unlimited);
its adapter is **unverified â€” assumes Tenor-compatible** request/response;
...
- `--type image|video` Â· `--count N` Â· `--json` Â· `--download --out DIR`
...
`{source, title, url, thumb, dl, page_url, author, license, w, h, type}` â€”
...
not single shots. So:
- **Modern single clip** â†’ `pexels` / `pixabay` (born as short clips, direct MP4). Done.
- **Historical single shot** â†’ retrieve the IA film here, then extract the shot:
  - **Twelve Labs** Marengo search (free 600 min) â€” pass the IA public MP4 URL, get a
...
- **`freesound-fetch.py "<query>" [count] [max_sec] [out_dir]`** â€” searches freesound.org
  ...
  (token-based read; full originals would need OAuth â€” previews suffice for SFX).
- **`audio-judge.py <file> "<target>"`** â€” sends the clip to OpenAI `gpt-audio`
  (audio-native) and returns JSON `{heard, score, matches, suggestion}`, enabling a
  generate/fetch â†’ judge â†’ iterate loop. Auto-sources a real `sk-` `OPENAI_API_KEY`
  ...
  like "grating" â€” it labels nearly any beep "sharp/high-pitched". Use it to cull, not
  to make the final aesthetic call; confirm by ear.

## Where this fits

This is the **internet-retrieval** capability â€” peer to `muser` (local semantic search)
