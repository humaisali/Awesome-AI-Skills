...
Analyze a user's competitors. Uses Browserbase Search API for discovery and a 4-lane Planâ†’Researchâ†’Synthesize pattern for enrichment â€” outputting an HTML report with overview, per-competitor deep dives, a side-by-side feature/pricing matrix, and a chronological mentions feed.
...
**Path rules**: Always use full literal paths in Bash â€” NOT `~` or `$HOME`. Resolve the home directory once and use it everywhere. When building subagent prompts, replace `{SKILL_DIR}` with the full literal path.
...
- Report compilation: use `node {SKILL_DIR}/scripts/compile_report.mjs {OUTPUT_DIR} --user-company "{user_company}" --open` â€” generates `index.html`, `competitors/*.html`, `matrix.html`, `mentions.html`, `results.csv` in one step and opens overview.
...
**CRITICAL â€” Minimize permission prompts**:
...
1. **User Company Research** â€” Deeply understand the user's company, produce `precise_category` + `category_include_keywords` + `exclusion_list`
2. **Depth Mode + Seed Input** â€” Choose depth, accept optional seed competitor URLs
3. **Discovery (3 parallel waves)** â€” Wave A (alternatives), Wave B (precise category), Wave C (comparison-page graph via "X vs Y" title parsing)
4. **Gate** â€” `scripts/gate_candidates.mjs` fetches each candidate's hero text (via `browse cloud fetch`) and drops wrong-category URLs
5. **Confirm enrichment set with the user** â€” Present PASS / UNKNOWN / rejected-brand-matches via `AskUserQuestion`. User ticks the real ones, adds any the discovery missed. Skipping this step is wasteful because enrichment is expensive (25 subagents Ã— depth budget) and the gate is imperfect (JS-heavy homepages, Cloudflare challenges, semantic-variant taglines)
6. **Deep Enrichment (5 subagents per competitor in deep/deeper modes)** â€” Marketing, Discussion, Social, News, Technical â€” each lane a separate subagent writing to `partials/`; then `merge_partials.mjs` consolidates. In deep/deeper modes, **Step 5d** adds a 6th Battle Card synthesis lane AFTER Step 5c fact-check completes â€” produces per-competitor Landmines / Objection Handlers / Talk Tracks grounded in cited evidence.
7. **Screenshots** â€” `capture_screenshots.mjs` via the `browse` CLI captures a 1280Ã—800 homepage hero per competitor
8. **HTML Report** â€” Overview + per-competitor (with embedded hero screenshot + Battle Card card) + matrix + mentions views
...
**Re-runs must start from a clean `$OUTPUT_DIR`.** `compile_report.mjs` ingests *every* `{slug}.md` in the directory, and `merge_partials.mjs` only overwrites the slugs in the current set â€” it never deletes ones dropped from a new enrichment set. Since the directory is keyed by date, a same-day re-run with a different competitor set would leave stale competitors in the overview, matrix, CSV, and screenshots. Either use a fresh directory or clear the prior per-competitor files first:
```bash
...
**Rule**: The user's company gets the same 5-lane research depth as competitors. Do NOT fill `userCompany` in matrix.json from memory â€” it will ship false claims to the user's own team. On a search-API run (user company Exa, 2026-04-23), skipping this step produced a matrix that claimed Exa had a "published uptime SLA" (there is no numeric public SLA â€” only a status page) and marked its MIT-licensed Python SDK as `open-source: false` (the repo is github.com/exa-labs/exa-py, LICENSE confirmed MIT). Both errors would have surfaced in the "Where you're winning" card as fabricated moats.

Process:

1. Ask the user for their company name or URL.

2. **Check for an existing profile** at `{SKILL_DIR}/profiles/{company-slug}.json`. If it exists, load it and confirm with the user: "I have your profile from {researched_at}. Still accurate?" â€” if yes, skip to Step 2 BUT still run the partial-lane enrichment below so matrix synthesis has fresh feature evidence.
...
3. **Run the full 5-lane enrichment on the user's company** â€” identical to the competitor pattern in Step 5. For each lane, spawn a Bash-only subagent that writes to `{OUTPUT_DIR}/partials/{user-slug}.{lane}.md`:
   - **marketing** â€” tagline, positioning, pricing tiers, features, integrations, open-source components (SDK repos + licenses), regions offered, compliance (SOC 2 / HIPAA / trust portal URL)
   - **technical** â€” REST + streaming API support (with docs URLs), SDK languages, MCP server URL, neural vs keyword retrieval modes, reranking / highlights / live-crawl specifics, published uptime SLA (actual %, not status page), third-party retrieval-quality benchmarks
   - **discussion**, **social**, **news** â€” optional in quick mode, recommended in deep+
   See `references/research-patterns.md` â†’ "Self-Research" for sub-questions. Each finding MUST cite a URL.

4. Run `merge_partials.mjs` on the user's partials too â€” produces `{OUTPUT_DIR}/{user-slug}.md`, the canonical source Step 5b reads from for `userCompany` flags.

5. Synthesize into a profile: Company, Product, Existing Customers, Competitors (seed list), Use Cases, **precise_category**, **category_include_keywords**, **exclusion_list**. Do NOT include ICP â€” this skill doesn't need it.
   ...
   - `exclusion_list`: phrases that indicate a *different* category â€” used by the gate to reject false positives (e.g. `antidetect browser`, `scraping api`, `screenshot api`, `residential proxy`).
   See `references/research-patterns.md` â†’ "Synthesis Output" for the exact format and Exa as a worked example.
...
- **Known competitors?** Text area for URLs/names (optional â€” discovery will find more).
- **Depth mode?**
  - `quick` â€” marketing surface only, many competitors, ~2-3 tool calls each
  - `deep` â€” + external signal (mentions, reviews, news), ~5-8 tool calls each
  - `deeper` â€” + public benchmarks + strategic diff vs user's company, ~10-15 tool calls each
...
Evaluation on a search-API run shows all three waves are additive â€” skip any and you lose real competitors:

**Wave A â€” Generic alternatives** (broad; heavy aggregator noise, filtered out later)
- `"alternatives to {user_company}"`
- `"{user_company} competitors"`

**Wave B â€” Precise category** (uses `precise_category` from the profile)
...
**Wave C â€” Comparison-page graph** (highest precision)
- `"{user_company} vs"`
- `"{seed1} vs"`, `"{seed2} vs"`, `"{seed3} vs"` (seeds from the profile's `competitors` list)
- After the searches, run `scripts/extract_vs_names.mjs` to parse `"X vs Y"` patterns from result titles â€” this uniquely surfaces competitors that don't appear as URL hits.

**Process**:
1. Issue **3 parallel `browse cloud search` Bash calls** (one per wave) in a SINGLE message â€” NOT subagents. Each Bash call chains its 2-4 queries with `&&`. See `references/workflow.md` â†’ "Discovery â€” parallel Bash, not subagents" for the exact recipe. Subagents are too heavy for a workload of 6-12 `browse cloud search` calls.
...
5. Merge with user-provided seed URLs. Dedup by hostname â†’ `/tmp/competitor_candidates.txt`.
...
The gate fetches each candidate's homepage via `browse cloud fetch --allow-redirects --format raw`, extracts the first 800 chars of visible text, and classifies position-aware: exclude in `<title>` â†’ REJECT; include in `<title>` â†’ PASS; hybrid title â†’ hero200 tiebreak; otherwise fall through.
...
Enrichment is expensive: 5 competitors Ã— 5 lane-subagents = 25 subagents, ~10-15 minutes of wall clock, ~300 `browse cloud` calls. Running it on the wrong set wastes all of that. The gate also has known blind spots:

- **JS-heavy homepages** (e.g. Tavily, Firecrawl) â€” `browse cloud fetch` returns near-empty text, so keyword matching has nothing to match on â†’ REJECT or UNKNOWN
- **Cloudflare challenge pages** (e.g. Perplexity) â€” title becomes "Just a moment..." â†’ no category signal
- **Semantic variants** â€” "search foundation" / "retrieval backbone" don't lexically match a list centered on "search API"
- **Domain ambiguity** â€” `brave.com` (the browser) vs `api-dashboard.search.brave.com` (the actual API product) can confuse classification

The user almost always has domain knowledge the skill lacks. Ask them.

**Process** â€” the main agent:

1. Read `/tmp/competitor_gated.jsonl` and group rows:
   - **PASS bucket**: everything with status=PASS.
   - **UNKNOWN bucket**: status=UNKNOWN (fetch failed â€” always surface, these are the silent misses).
...
4. Write the confirmed set to `/tmp/competitor_enrichment_set.txt` (one URL per line). This is the input for Step 5 â€” not `/tmp/competitor_passed.txt`.
...
**Exa test, 2026-04-24**: gate auto-passed 22 of 101 candidates but missed Tavily (generic title), Jina AI (semantic mismatch â€” "search foundation"), Firecrawl (JS-heavy fetch failure), and Perplexity (Cloudflare challenge). All four are real direct competitors. This step catches them.
...
### Quick mode â€” single subagent per batch
...
### Deep / Deeper mode â€” 5 subagents PER competitor (parallel lane fan-out)
...
- **B. Discussion** (`discussion`): Reddit, HN, forums, Dev.to, Hashnode. Broad queries beyond `site:` â€” also `"{competitor}" review 2026`, `"{competitor}" issues OR problems`, `"{competitor}" discussion`.
- **C. Social** (`social`): LinkedIn posts, YouTube videos, Twitter/X. Snippets only â€” do NOT fetch.
...
**Launch ALL competitor Ã— lane subagents in a SINGLE Agent tool message.** For 10 competitors Ã— 5 lanes = 50 parallel Agent calls in one message. Do NOT split into batches per competitor or per lane â€” wall clock collapses to the slowest single agent (~3-5 min). Splitting into 5 rounds of 10 cost 25 minutes of wall clock vs 5 minutes parallel on a real measured run; do not do it.
...
### Merge partials â†’ canonical per-competitor file
After all subagents for all competitors complete:
```bash
node {SKILL_DIR}/scripts/merge_partials.mjs {OUTPUT_DIR}
```
Unions the 5 partials per competitor into one `{OUTPUT_DIR}/{slug}.md` â€” dedup'd Mentions (sorted by date desc), dedup'd Benchmarks, merged Findings, canonical frontmatter from the marketing lane.

### Synthesize the comparison matrix (write `matrix.json`)

**Subagents write `key_features` and `integrations` as prose**, not as pipe-separated atomic feature labels. So a naive `|`-split axis becomes one-blob-per-competitor with no overlap â€” the rendered matrix shows a useless diagonal.
...
**Process** â€” main agent:
1. Read ALL `{slug}.md` files, INCLUDING the user's company file `{user-slug}.md` produced in Step 1. The user is competitor #0 for matrix purposes â€” treat with identical rigor.
2. Produce a canonical list of 12-20 *atomic* features â€” each must be a yes/no proposition a competitor either has or doesn't (e.g. "MCP server", "SOC 2", "Site crawler", "Reranker"). Avoid sentence-length features. Avoid features only one competitor has.
...
5. Write the result to `{OUTPUT_DIR}/matrix.json` in this shape:
   ```json
   {
     ...
     "userCompany": {
       "name": "Exa",
       "winningSummary": "Exa's moats are its first-party neural index and the integrated Research API â€” no one else in the set ships a semantic/embeddings-native retrieval primitive alongside a multi-step agentic research endpoint. It's also the only provider with a crawler product bundled in, and ties with SerpAPI on breadth of SDK language coverage.",
       "losingSummary": "Exa trails competitors on operational transparency â€” SerpAPI, Serper, and Tavily all publish hourly throughput SLAs, and Exa lacks a dedicated news endpoint that SerpAPI, Serper, and You.com all ship. Image/visual search is also missing vs 4 of 5 competitors.",
...
### Fact-check the matrix â€” spot-check the high-stakes cells (default)
...
But verifying every cell is the opposite mistake. A 7-company Ã— 33-axis matrix has 231 cells. The Apr 2026 search-API run got stuck at 111+ tool calls in fact-check before interrupt â€” the subagent kept going on table-stakes cells (REST API, JSON responses, Python SDK) that are universal in the category.
...
1. **Every `userCompany.features` and `userCompany.integrations` cell** (the user's own moats â€” these go straight into "Where you're winning" prose). Typical: 17 + 16 = 33 cells, but most are obvious (your own product). Focus on:
   ...
   - Open-source license claims (MIT / Apache 2.0 / AGPL â€” observed wrong on a competitor's SDK)
   - Published uptime SLA (status page â‰  SLA)
...
3. **Do NOT verify**:
   - Universal table-stakes (REST API, JSON responses, Python SDK, API-key auth) â€” every search API has these.
...
Stop and return what you have when you hit the budget â€” partial fact-check is
...
PRIORITY ORDER (highest-stakes first â€” work down until budget):
...
- If `true` â€” find one source URL (docs, trust portal, GitHub LICENSE, etc).
- If `false` â€” one targeted browse cloud search. Flip ONLY on first-party evidence.
...
After the subagent completes, re-read matrix.json, recompile, and surface `matrix_fact_check.md` delta to the user. The summary is much more trustworthy with spot-check than without â€” and ships in 3-5 minutes instead of stalling the pipeline.
...
**Why this lane is synthesis-only** â€” battle cards must be grounded in facts that already survived Step 5c. Letting the subagent do fresh `browse cloud` searches would reintroduce the hallucinated-moat problem the fact-check step exists to prevent. The subagent's adversarial self-check explicitly rejects claims not traceable to an input partial bullet or a `sources`-backed matrix cell.

Parallelism: 1 subagent per competitor, all in one Agent-tool message (synthesis is fast, ~3-5 Bash calls per subagent). Skip this step in `quick` mode â€” there isn't enough research depth to ground the cards credibly.
...
Uses the `browse` CLI (`npm install -g browse`). The `--mode` flag selects the browser session: `remote` (default) drives a Browserbase session â€” best for protected/bot-detecting homepages and the only option without local Chrome; `local` uses Chrome on your machine. The script passes the corresponding `--remote` / `--local` flag on each `browse` command, so there is no separate environment-config step to run. Writes one PNG per competitor to `{OUTPUT_DIR}/screenshots/{slug}-hero.png`. The compile step in Step 7 auto-embeds the hero on each per-competitor HTML page.
...
1. **Generate all views + CSV** (opens overview in browser):
   ...
   ```
   Produces:
   - `{OUTPUT_DIR}/index.html` â€” overview: competitor table with tagline, pricing summary, key features, strategic diff
   - `{OUTPUT_DIR}/competitors/{slug}.html` â€” per-competitor deep dive (all sections)
   - `{OUTPUT_DIR}/matrix.html` â€” side-by-side feature/pricing matrix
   - `{OUTPUT_DIR}/mentions.html` â€” chronological feed with source-type pills + client-side filter
   - `{OUTPUT_DIR}/results.csv` â€” flat spreadsheet
...
4. Call out the top 3-5 most interesting findings â€” e.g., "3 competitors have public benchmarks; Rival Co is cheapest; Foo Inc launched a dedicated news-search endpoint 2 weeks ago." Offer to dig deeper into any specific competitor or re-run with different depth.
