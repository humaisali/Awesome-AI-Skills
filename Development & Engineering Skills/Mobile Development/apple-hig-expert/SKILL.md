...
- **Mode 1 â€” Design from scratch**: pick the platform navigation paradigm and layout primitives first (see `references/platform-specifics.md`), then apply typography and semantic color (`references/visual-design.md`).
- **Mode 2 â€” HIG audit**: fill in `templates/hig-audit-template.md`, run `scripts/hig_checker.py` on every measurable element, and deliver a scored report (see Worked example below).
...
**Findings â†’ fixes (bottom line first):**

> **HIG score 80/100 â€” two fixes before release.**
> 1. Captions fail contrast (3.26 < 4.5). Use `.secondaryLabel` (semantic color) instead of hardcoded `#8E8E93`, or darken to â‰¥ `#6E6E73` on white. ðŸŸ¢ verified by tool.
> 2. Close button is 32x32 pt (< 44x44 minimum). Keep the glyph small but expand the hit region to 44x44 with padding/`contentShape`. ðŸŸ¢ verified by tool.
> 3. Manual check: the card uses an ultra-thin material over a photo background â€” re-test caption contrast against the *busiest* underlying region and with Reduce Transparency on. ðŸŸ¡ needs device test.

## Core Design Principles

1. **Liquid Glass** â€” translucent material hierarchy (announced at WWDC25, June 2025; shipped Sept 2025 across iOS 26, iPadOS 26, macOS Tahoe, watchOS 26, tvOS 26, visionOS 26). In SwiftUI, apply it via the `glassEffect` view modifier; keep hierarchy between content and controls. See `references/visual-design.md`.
2. **Accessibility first** â€” VoiceOver labels on every element, 44x44 pt minimum targets, 4.5:1 contrast for normal text (3:1 large text), Dynamic Type support. See `references/accessibility.md`.
3. **Platform ergonomics** â€” tab bars/thumb reach on iOS, sidebars + menu bar + shortcuts on macOS, ornaments + gaze states on visionOS, glanceable vertical layouts on watchOS. See `references/platform-specifics.md`.
...
- **Bottom line first** â€” compliance status before details.
- **What + Why + How** â€” "Expand the hit region (What) because 32 pt targets fail the HIG minimum (Why); pad to 44x44 via contentShape (How)."
- **Confidence tagging** â€” ðŸŸ¢ tool-verified / ðŸŸ¡ needs device test / ðŸ”´ assumed.
