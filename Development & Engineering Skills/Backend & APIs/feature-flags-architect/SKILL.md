...
```
request â†’ design â†’ ship â†’ ramp â†’ cleanup â†’ archive
```
...
|---|---|---|---|---|
| **Release** | Hide unfinished features in production | daysâ€“weeks | Eng | 100% rollout reached |
| **Experiment** | A/B test variants | weeks | Product/Marketing | Test concluded; winner picked |
| **Operational** | Circuit breakers, perf toggles, kill switches | monthsâ€“years | Eng/SRE | Replaced by autoscaling/feature retirement |
...
3. Flag as DEBT if introduced > `--max-age-days` ago AND used in â‰¤`--min-uses` places.
...
**Strategies:**
- `ring`: 1% â†’ 5% â†’ 25% â†’ 50% â†’ 100%, evenly spaced. Default for risky launches.
...
- `cohort`: by named cohort (internal â†’ beta â†’ free â†’ paid â†’ all).
...
Decision rules:
- <50 flags + no targeting â†’ DIY with config file or env vars
- Need analytics + experimentation â†’ Statsig or GrowthBook
- Compliance/SOC2 audit logs required â†’ LaunchDarkly
- Self-hosting required (data residency / air-gapped) â†’ Unleash or Flipt
...
1. Classify: which of the 4 flag types?
   â†’ Release (most common for engineering work)
...
5. Run kill_switch_audit.py â€” must pass before merge
...
2. For each flagged item:
   ...
   d. Run kill_switch_audit.py â€” should now show one fewer flag
...
- `references/flag_taxonomy.md` â€” 4 types, decision tree, ownership, lifespan
- `references/provider_comparison.md` â€” LaunchDarkly / GrowthBook / Statsig / Unleash / Flipt / DIY trade-offs
- `references/rollout_strategies.md` â€” ring / linear / log / cohort / geo, abort criteria, monitoring
- `references/flag_lifecycle.md` â€” request â†’ design â†’ ship â†’ ramp â†’ cleanup â†’ archive

## Slash command

`/flag-cleanup` â€” Run the full cleanup workflow on the current repo: scan for debt, generate a removal plan, audit kill switches.

## Asset templates

- `assets/flag_request_template.md` â€” fill-in form for new flag requests (name, owner, type, kill switch, rollout plan)

## Anti-patterns

- **Permanent flag with `if (FLAG_FOO)` 50 places** â€” should be a Permission flag with a runtime config, not a Release flag
- **Flag with no owner** â€” when the original engineer leaves, no one cleans it up
- **No kill switch documented** â€” when the feature breaks, no one knows how to disable it
- **A/B test that ran 6 months** â€” pick a winner; running indefinitely is debt
- **Flags as feature toggles for cosmetic changes** â€” ship via deploy, not flag

## Verifiable success

A team using this skill should achieve:
- 100% of new flags pass `kill_switch_audit.py` at merge time
- `flag_debt_scanner.py --max-age-days 90` returns â‰¤5 stale flags repo-wide
