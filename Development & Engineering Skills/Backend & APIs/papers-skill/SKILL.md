...
absolute interpreter path â€” and remember to invoke pip via the same
...
Summarize as: **problem Â· method Â· key result Â· limitations**.
...
- âœ… Always call `detail` before `download` to confirm the paper matches user
  intent. Skipping this leads to wrong PDFs being fetched.
- âœ… Include the paper ID alongside every title in your output so the user
  can re-query precisely.
- âœ… Cite as `[FirstAuthor et al., Year] *Title* (cites: N)`.
- âœ… For PDFs you download, always report the absolute save path.
- âŒ Don't crawl. The script auto-retries 429s with exponential backoff;
  don't pile on parallel queries.
- âŒ Don't raise `--max-pages` to 100+ without warning the user â€” it can
...
- PyMuPDF extracts embedded text only. Scanned image-PDFs return the
  fallback message `PDFæ— æ³•æå–æ–‡æœ¬ï¼ˆå¯èƒ½æ˜¯æ‰«æä»¶ï¼‰`; offer the user an
  alternative version or note that OCR is required.
- Semantic Scholar's anonymous tier rate-limits aggressively. The script
  retries 3Ã— with exponential backoff; persistent 429s during heavy use
  surface as `æœç´¢å¤±è´¥: rate limit, retries exhausted`.
...
- `read` opens a local PDF file with PyMuPDF â€” make sure the path the user
  supplies is one they trust.
- No credentials or API keys are needed or stored anywhere.

## Common Pitfalls

- **Problem:** `éœ€è¦å®‰è£… arxiv: pip install arxiv` or `éœ€è¦å®‰è£… PyMuPDF: pip install PyMuPDF`.
...
- **Problem:** `æœç´¢å¤±è´¥: rate limit, retries exhausted` from `search` or
  `detail` or `citations`.
...
- **Problem:** `download` fails with `æ‰¾ä¸åˆ° arXiv ID: â€¦`.
