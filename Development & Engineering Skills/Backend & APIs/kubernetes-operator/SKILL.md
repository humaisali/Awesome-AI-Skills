...
description: Use when building a Kubernetes Operator â€” custom controllers that reconcile CRD state. Triggers on "build an operator", "CRD design", "reconcile loop", "controller-runtime", "kubebuilder", "operator-sdk", "metacontroller", "KOPF", "operator capability levels", or "custom resource". Ships CRD validator, reconcile-loop linter, and OperatorHub capability auditor (all stdlib Python), 4 references on the operator pattern + CRD design + reconcile patterns + tooling landscape, and a /operator-audit slash command. NOT a generic k8s skill â€” specifically the Operator pattern.
context: fork
version: 2.9.0
...
Build operators that reconcile correctly. Most operator bugs are not Kubernetes bugs â€” they are reconcile-loop bugs: missing finalizers, blocking calls, no requeue on transient errors, status drift, RBAC over-grants. This skill catches them deterministically before they reach a cluster.
...
- Plain Helm chart packaging â†’ use `helm-chart-builder`
- Standard kubectl operations / blue-green deploys â†’ use `senior-devops`
- General k8s security posture â†’ use `cloud-security`
- "I want to run a workload" â€” that's a Deployment / Job, not an operator

## Core principle: an operator is a reconcile loop, not a script

```
observe(actual) â†’ desired = read(spec) â†’ diff(actual, desired) â†’ act â†’ update(status)
                                                                          â†“
                                                                   requeue / done
```
...
5. Don't use the status subresource (status updates trigger spec reconciles â†’ loop)
6. Block in reconcile (long HTTP calls, locks)
7. Forget leader election â†’ split-brain on multi-replica deploys
...
**Levels:**
- **L1 â€” Basic Install:** CRD defined, controller deploys it
- **L2 â€” Seamless Upgrades:** PDBs, conversion webhooks, version skew strategy
- **L3 â€” Full Lifecycle:** backups, restores, failure recovery
- **L4 â€” Deep Insights:** metrics endpoint, Prometheus rules, alerts
- **L5 â€” Auto Pilot:** auto-scaling, auto-tuning, anomaly detection
...
Decision rules:
- New operator + Go shop â†’ kubebuilder
- New operator + Python shop â†’ KOPF
- New operator + can't pick a language â†’ metacontroller
- OpenShift target â†’ operator-sdk
...
5. **Version your CRD from day 1.** `v1alpha1` â†’ `v1beta1` â†’ `v1`. Plan a conversion webhook.
...
1. **Idempotent.** Reconciling the same state twice â†’ same result, zero side effects.
...
4. Run crd_validator.py on config/crd/bases/apps.example.com_myapps.yaml
   â†’ Fix every WARN before writing controller code
...
7. Run operator_capability_audit.py --operator-dir . â€” confirm L1
...
4. Triage findings:
   - FAIL â†’ block release; fix before next deploy
   - WARN â†’ file an issue; fix in next 30 days
...
- `references/operator_pattern.md` â€” what an operator IS, when to use vs alternatives
- `references/crd_design.md` â€” CRD design principles, versioning, conversion webhooks
- `references/reconcile_loop.md` â€” reconcile patterns, error handling, idempotency
- `references/tooling_landscape.md` â€” framework comparison + decision tree

## Slash command

`/operator-audit` â€” Run all 3 tools on an operator repo and produce a markdown report.

## Asset templates

- `assets/crd_template.yaml` â€” CRD with status subresource, conditions, finalizer hint, printer columns
- `assets/reconcile_skeleton.go` â€” Go controller reconcile function with idempotency, conditions, finalizers, requeue patterns

## Anti-patterns

- **`time.Sleep(30 * time.Second)` inside reconcile** â€” block other reconciles. Use `RequeueAfter`.
- **`r.Client.Update(ctx, obj)` to set status** â€” use `r.Status().Update(ctx, obj)` instead.
- **No leader election + 2+ replicas** â€” split-brain.
- **No finalizer** â€” external resources orphan on deletion.
- **CRD without status subresource** â€” status updates trigger spec reconciles (infinite loop).
- **Reconcile function > 200 lines** â€” extract reconcileXxx subroutines per condition.
- **`x-kubernetes-preserve-unknown-fields: true` on spec root** â€” defeats validation.
- **Imperative reconcile** â€” "if creating, do A; if updating, do B; if deleting, do C". Wrong shape. Reconcile = make actual=desired, regardless of how we got here.
