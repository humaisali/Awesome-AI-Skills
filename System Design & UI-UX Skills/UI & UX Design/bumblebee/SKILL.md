...
5. Summarize findings â€” especially exposure-catalog matches â€” in the chat reply.
...
## Step 1 â€” Clarify the scan request
...
If the user has an advisory or exposure-catalog file ready, also ask whether they want to pass it via `--exposure-catalog`. The skill does not ship its own catalogs â€” point them at `threat_intel/` in the Bumblebee repo if they ask where to find ready-made ones.

Skip the questions for one-liner asks like "lauf mal ne Baseline-Scan" â€” just run a baseline.

## Step 2 â€” Check Go

Run `command -v go && go version` in bash. Three outcomes:

- **Go â‰¥ 1.25 present** â†’ continue.
- **Go present but < 1.25** â†’ tell the user the version, explain Bumblebee needs Go 1.25+, and stop until they upgrade.
- **Go missing** â†’ do not install Go automatically. Show platform-appropriate instructions and stop:
...
## Step 3 â€” Check or install Bumblebee
...
Then re-check `bumblebee version`. If the binary still cannot be located, the user's `GOBIN`/`PATH` is likely misconfigured â€” surface the resolved `go env GOPATH` and `go env GOBIN` so they can fix it. Do not fall back to running the binary by absolute path silently; explain what is happening.
...
- `bumblebee-<profile>-<UTC-timestamp>.ndjson` â€” raw records.
- `bumblebee-<profile>-<UTC-timestamp>.report.md` â€” Markdown report (generated in Step 5).
...
Always stream stderr to a sibling `.log` file â€” Bumblebee emits diagnostic NDJSON there that helps explain partial scans.
...
Each `--root` must be an existing absolute path. Reject bare `$HOME` for this profile (Bumblebee will reject it too â€” surface the message clearly).
...
Used for incident response â€” broad roots are allowed but should be paired with an exposure catalog and `--findings-only` whenever possible, so the output stays focused.
...
## Step 5 â€” Generate the Markdown report
...
The helper groups records by type and ecosystem, lists every `finding` record with its catalog entry and severity, and embeds the `scan_summary` for traceability. It is dependency-free Python 3 â€” no `pip install` needed.
...
## Step 6 â€” Present results

End the turn with:

- A short summary in chat: profile, root(s), record counts, and â€” most importantly â€” any findings with their severity. If there are zero findings, say so explicitly; silence on findings is the kind of thing that gets misread.
...
Do not paste large chunks of NDJSON into the chat â€” it is noisy and not where the user will read it.
...
- `bumblebee: command not found` after `go install` â†’ almost always a `PATH`/`GOBIN` problem. Show `go env GOPATH GOBIN PATH` to debug.
- `refusing to scan bare home with profile baseline` â†’ use `deep` for `$HOME`, or pick a subdirectory for `project`.
- Scan times out â†’ either narrow the `--root` set, scope with `--ecosystem`, or raise `--max-duration`. Do not loop and retry blindly.
- Exposure catalog rejected â†’ check that the JSON has both `schema_version` and `entries` keys (bare top-level arrays are rejected) and that `schema_version` is one Bumblebee understands.
...
See `scripts/render_report.py` for the report layout. Bumblebee's own documentation lives at https://github.com/perplexityai/bumblebee â€” consult `docs/inventory-sources.md`, `docs/transport.md`, and `docs/state-model.md` when a question goes beyond what this skill covers.
