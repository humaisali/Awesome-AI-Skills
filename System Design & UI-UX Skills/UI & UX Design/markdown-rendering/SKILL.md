...
`cmux markdown open` defaults to **spawning a brand-new pane** every time, even with `--direction right`. The common "fix" â€” moving the new markdown surface into the existing right pane with `move-surface` â€” **bugs out: the moved viewer renders BLANK.** The surface keeps `type=markdown` and looks healthy, but shows nothing.

So you get stuck: either a stray extra pane, or a blank viewer after moving it.

## The Rule

You have exactly two reliable options. **Never `move-surface` a markdown viewer** â€” that is the path that bugs.

### Option A â€” Open it right on the first try
...
### Option B â€” Close existing right pane(s), then open fresh
...
# 3. THEN open the markdown fresh â€” it creates its own clean right pane
cmux markdown open /abs/path/file.md --direction right --focus false
```
...
- Only close panes that are unused or irrelevant â€” never close a pane the user is working in.
